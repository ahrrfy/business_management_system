...
export const purchaseRouter = router({
  /**
   * مرجع تسعير حيّ لمحرّر الشراء. لا يعتمد على costPrice الحالي لأنه قد يتغيّر يدوياً؛
   * المصدر هو بنود أوامر الشراء المعتمدة/المستلمة فقط. نعيد آخر سعر وأقل سعر، وكذلك
   * آخر سعر من المورد المحدّد كي يستطيع الموظف اتخاذ قرار واعٍ قبل الحفظ.
   */
  priceInsights: purchasesManagerProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      // نفس سياسة الإنشاء: مسؤول المشتريات لا يطّلع إلا على تاريخ فرعه، أما المدير/الأدمن
      // فيستطيعان المقارنة في الفرع الذي اختاراه في رأس الفاتورة.
      ...
      const uniqueItems = Array.from(new Map(input.items.map((item) => [`${item.variantId}:${item.productUnitId}`, item])).values());
      const itemConditions = uniqueItems.map((item) => and(
      ...
      const rows = await db
        ...
        .where(and(
          eq(purchaseOrders.branchId, effectiveBranchId),
      ...
      const result: Record<string, {
        ...
        lowestPurchase: PurchaseReference;
        selectedSupplierLastPurchase?: PurchaseReference;
      }> = {};
      ...
      for (const row of rows) {
        ...
        if (input.supplierId === reference.supplierId && !current.selectedSupplierLastPurchase) {
