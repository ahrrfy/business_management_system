...
describe("csrfGuard — متصفحات تحجب Origin/Referer (علّة اللوحي)", () => {
  ...
  it("يرفض Sec-Fetch-Site: same-site (نطاق شقيق ليس نفس الأصل)", () => {
    const { res, nextCalled } = run(mockReq({ headers: { "sec-fetch-site": "same-site" } }));
    expect(nextCalled).toBe(false);
    expect(res.statusCode).toBe(403);
  });
...
describe("csrfGuard — شكل الرفض بحسب السطح", () => {
  // بعد سماح الغياب التام (٦/٧) صار مُطلِق الرفض للاختبار هو شهادة cross-site الصريحة.
  it("على /api/trpc: غلاف tRPC يجتاز تحويل العميل ويعرض الرسالة العربية", () => {
    const { res } = run(
      mockReq({ baseUrl: "/api/trpc", headers: { "sec-fetch-site": "cross-site" } })
    );
    const transformed = transformResult(
      res.body as Parameters<typeof transformResult>[0],
      superjson
  ...
  it("على /api/print: الشكل العاري القديم {error} كما كان", () => {
    const { res } = run(
      mockReq({ baseUrl: "/api/print", headers: { "sec-fetch-site": "cross-site" } })
    );
    expect(res.statusCode).toBe(403);
