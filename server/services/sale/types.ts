...
export interface SaleLineInput {
  ...
  /**
   * **تصدير داخليّ بحت — لا يقبله أيّ راوتر** (نفس نمط `unitCostOverride`).
   * سعرُ المرجع المفروض خادمياً لهذا السطر، يتقدّم على سعر القائمة في **القياس** (لا في السعر
   * المُحصَّل). مستهلكه الوحيد اليوم: البيع الرقميّ — عرضُ البطاقة يُنشئ في `productPrices`
   * صفّاً نائباً بسعر `0` عمداً («real price comes from digital pricing»)، وسعرُها الحقيقيّ
   * `sellPriceSnapshot` مقروءٌ من نيّةٍ **مقفولة** في القاعدة. بدونه يسقط بيعُ البطاقات كلّه
   * على حارس H7 (`requireListPriceReference`) بحجّة «بلا سعر قائمة»، وهي حجّةٌ كاذبة هنا.
   */
  unitPriceReference?: string | null;
  /**
