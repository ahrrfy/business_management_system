...
export interface CashRemediationSnapshotProbe {
  /** اختبار تكامل فقط: يوقف القراءة بعد أول SELECT لإدخال كتابة متزامنة من اتصال آخر. */
  afterShiftRead?: () => Promise<void>;
}

/**
 * تقرير تشخيص تاريخي فقط. كل الاستعلامات داخل لقطة REPEATABLE READ/READ ONLY واحدة؛
 * لا insert/update/delete ولا استدعاء خدمة ترحيل.
...
export async function getCashRemediationDryRun(
  ...
  snapshotProbe?: CashRemediationSnapshotProbe,
  ...
  return database.transaction(
    async (db) => {
      ...
      await snapshotProbe?.afterShiftRead?.();
