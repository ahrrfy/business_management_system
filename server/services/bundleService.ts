...
async function lockBundleVariantScope(
  ...
  if (missing.length) {
    throw new TRPCError({
      ...
      message: appErrorMessage({
        what: "تعذّر تثبيت مكوّنات البكج",
        why: `المتغيّرات التالية غير موجودة: ${missing.map((id) => `#${id}`).join("، ")}`,
        doThis: "حدّث تعريف البكج واحذف المكوّنات المحذوفة ثم أعد الحفظ",
      }),
    });
  }
  ...
  for (const variant of lockedVariants) {
    ...
    if (refByVariant.get(id) !== productId) {
      throw new TRPCError({
        code: "CONFLICT",
        message: appErrorMessage({
          what: "تعذّر تثبيت تعريف البكج",
          why: "تغيّر ربط أحد المتغيّرات بمنتجه أثناء الحفظ",
          doThis: "حدّث الصفحة وراجع المكوّنات ثم أعد المحاولة",
        }),
      });
    }
    ...
    if (!product) {
      throw new TRPCError({
        ...
        message: appErrorMessage({
          what: "تعذّر تثبيت تعريف البكج",
          why: "حُذف منتج أحد المتغيّرات أو تغيّر أثناء الحفظ",
          doThis: "حدّث الصفحة واستبدل المكوّن غير المتاح ثم أعد المحاولة",
        }),
      });
    }
    out.set(id, {
  ...
  if (out.size !== ids.length) {
    throw new TRPCError({
      code: "CONFLICT",
      message: appErrorMessage({
        what: "تعذّر تثبيت نطاق البكج",
        why: "تغيّرت قائمة المتغيّرات أثناء قفلها للحفظ",
        doThis: "حدّث الصفحة وراجع قائمة المكوّنات ثم أعد المحاولة",
      }),
    });
  }
  return out;
...
export async function replaceBundleComponents(
  ...
  if (
    ...
    throw new TRPCError({
      ...
      message: appErrorMessage({
        ...
        why: "عدّل مستخدم أو عملية أخرى مكوّنات البكج أثناء الحفظ",
        doThis: "حدّث الصفحة وراجع المكوّنات الحالية ثم أعد المحاولة",
      }),
    });
  }
  ...
  if (parent.productActive !== true || parent.variantActive !== true) {
    throw new TRPCError({
      ...
      message: appErrorMessage({
        what: "تعذّر تعديل تعريف البكج",
        why: "منتج البكج أو متغيّره معطّل",
        doThis: "فعّل المنتج ومتغيّره أولاً ثم أعد حفظ المكوّنات",
      }),
    });
  }
