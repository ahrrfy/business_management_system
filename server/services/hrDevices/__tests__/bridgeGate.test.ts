...
afterEach(async () => {
  ...
  if (original.allowlist === undefined) delete process.env.HR_DEVICE_IP_ALLOWLIST;
  ...
  if (original.identityBindings === undefined) delete process.env.HR_DEVICE_IDENTITY_BINDINGS;
  else process.env.HR_DEVICE_IDENTITY_BINDINGS = original.identityBindings;
  if (original.legacyMigration === undefined) delete process.env.HR_DEVICE_LEGACY_IDENTITY_MIGRATION;
  else process.env.HR_DEVICE_LEGACY_IDENTITY_MIGRATION = original.legacyMigration;
...
function httpStatus(port: number, path: string): Promise<{ status: number; body: string }> {
...
describe("بوابة شبكة جسر الحضور", () => {
  it("لا تفتح المنفذ في الإنتاج عند غياب ضوابط المصدر", () => {
  ...
  it("لا تفتح المنفذ في الإنتاج بقائمة شاملة أو سر دوري", () => {
    process.env.NODE_ENV = "production";
    process.env.HR_DEVICE_IP_ALLOWLIST = "0.0.0.0/0";
    delete process.env.HR_DEVICE_SHARED_SECRET;
    expect(startHrDeviceBridge(0)).toBeNull();
  ...
  it("ترفض HTTP بلا سر وتقبل ZK بالسر دون تغيير البروتوكول", async () => {
    ...
    const accepted = await httpStatus(port, "/iclock/cdata?token=test-gateway-secret");
  ...
  it("ترفض WebSocket بلا سر وتقبل Aiface بالسر", async () => {
    ...
    const accepted = new WebSocket(`ws://127.0.0.1:${port}/?token=test-gateway-secret`);
