...
export async function getTreasuryStatement(opts: {
  ...
  const openingBalance = money(openRow.opening ?? 0);
  ...
  const totalIn = money(aggRow.totalIn ?? 0);
  const totalOut = money(aggRow.totalOut ?? 0);
  const closingBalance = openingBalance.plus(totalIn).minus(totalOut);

  // (ج) تفصيل الحركات مرتّباً زمنياً — رصيدٌ جارٍ يُراكَم من الافتتاحيّ.
  const rows = rowsOf(
    await db.execute(sql`
      SELECT
        r.id AS receiptId,
        ...
        r.counterpartyName AS counterparty,
  ...
  const movements: TreasuryStatementMovement[] = rows.map((r) => {
    const amt = money(r.amount ?? 0);
