...
describe("INVOICE digital intent source payload", () => {
  ...
  it("يحصر إيصال الدفع الدائم في CASH أو CARD ويسمح بتعافي آجل تاريخي بلا قبض", () => {
    expect(() => parseInvoiceSourcePayload({
      ...rawPayload(),
      payment: { amount: "240.00", method: "CREDIT" },
    })).toThrow(/حمولة فاتورة البيع الرقمية غير صالحة/);

    const { payment: _payment, ...historicalRaw } = rawPayload();
    const historical = parseInvoiceSourcePayload(historicalRaw);
    expect(() => assertInvoiceFullPayment(historical, {
    ...
    expect(() => assertInvoiceFullPayment(historical, {
      paymentMethod: "CREDIT",
      paymentAmount: "1.00",
    })).toThrow(/يجب ألا تتضمن مبلغاً مقبوضاً/);
  });
