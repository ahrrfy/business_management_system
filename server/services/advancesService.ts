...
/**
 * #6 (تدقيق التثبيت): استعادة أرصدة السلف عند عكس مسيّر مدفوع.
 * السياق: كان cancelRun(paid→approved) لا يُعيد `remaining` المُنقَصة (بحسب قرار موثَّق قديم).
 * لكنّ الحذف اللاحق للمسيّر ثم توليد مسيّر جديد لنفس الفترة يُنتج **خصم سلفة مضاعف** —
 * لأن `isFirstPay` في `payRun` يُفحَص بمُعرِّف المسيّر لا بالفترة/الموظف. الحلّ الصحيح: عكس
 * الدفع يستعيد الرصيد (يطابق دلالة العكس المحاسبيّ: النقد عاد للخزينة ⇒ الاستقطاع لم يقع فعلياً).
 *
 * منطق الاستعادة: LIFO عكسياً على استقطاع الدفع — نضيف المبلغ لأحدث سلفة نشطة أوّلاً حتى بلوغ
 * `amount` الأصلي، ثم للأحدث SETTLED نُعيد تفعيلها بـremaining=take. نُلتقط تقريب الفلس بحاشية
 * دنيا (٠.٠١) على السلفة الأخيرة كي لا نتجاوز `amount` بسبب round-trip decimal.
 */
export async function restoreAdvancesOnCancelTx(
  tx: Tx,
  ...
  for (const item of items) {
    let left = round2(item.amount);
    if (left.lte(0)) continue;
    // LIFO: نبدأ من الأحدث. ACTIVE أولاً (نملأ حتى amount) ثم SETTLED (نُعيد التفعيل).
    const advs = await tx
      .select()
      .from(employeeAdvances)
      .where(and(eq(employeeAdvances.employeeId, item.employeeId), inArray(employeeAdvances.status, ["ACTIVE", "SETTLED"])))
      .orderBy(desc(employeeAdvances.id))
      .for("update");
    for (const adv of advs) {
      if (left.lte(0)) break;
      const original = money(adv.amount);
      const remaining = money(adv.remaining);
      // كم يمكن أن نستعيد على هذه السلفة قبل تجاوز مبلغها الأصلي.
      const capacity = original.minus(remaining);
      if (capacity.lte(0)) continue;
      const give = Decimal.min(capacity, left);
      const newRemaining = round2(remaining.plus(give));
      left = round2(left.minus(give));
      await tx
        .update(employeeAdvances)
        .set({
          remaining: toDbMoney(newRemaining),
          status: newRemaining.gt(0) ? "ACTIVE" : "SETTLED",
        })
        .where(eq(employeeAdvances.id, Number(adv.id)));
    }
    // متبقٍّ لم يُستعَد ⇒ سلف الموظف كلها ممتلئة أصلاً؛ نتجاهل بصمت (لا يجب أن يحدث في الممارسة —
    // لو حدث فالسلف كانت في وضع لا نستطيع فيه استعادة أكثر، وهذا ليس عطلاً يستحقّ إحباط العكس).
  }
}
