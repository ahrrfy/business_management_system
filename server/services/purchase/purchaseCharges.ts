...
export async function decidePurchaseChargeControl(input: DecidePurchaseChargeControlInput, actor: Actor) {
  ...
  return withTx(async (tx) => {
    ...
    assertApprover({ actor: purchaseChargeApprover, trigger: purchaseChargeControlTrigger(input.action), subject: `مصروف شراء (طلب ${input.requestId})`, legacy: () => { if (purchaseChargeApprover.isOwner) return; assertIndependentPurchaseReviewer(Number(request.requestedBy), actor.userId); } });
