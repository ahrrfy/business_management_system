...
export async function createPurchaseOrder(input: CreatePurchaseOrderInput, actor: Actor) {
  return withTx(async (tx) => {
    ...
    // قرار المالك (٥/٨/٢٦): **إجماليّ أمر الشراء = ذمّة المورّد = البضاعة + الضريبة فقط.**
    // الشحن/الكمرك يبقيان مخزَّنَين على الأمر (تقديرُ ما سنَدفعه لشركة النقل، ويُوزَّع على البنود
    // بنسبة القيمة لأغراض العرض والتقارير) لكنّهما **خارج الإجمالي وخارج الذمّة** — يُسجَّلان
    // مصروفَ نقلٍ مستقلاً لحظة الاستلام (`receive.ts`). إبقاؤهما في total كان يجعل «المتبقّي على
    // الأمر» و«سقف الدفعة» يطالبان المورّد بمالٍ لم يبعه لنا.
