...
export function cumulativePurchaseTax(
  ...
  const tax = fullyReceived
    ? round2(money(poTax).minus(money(priorTax)))
    : round2(receivedNet.times(rate));
  if (tax.lt(0) || money(priorTax).plus(tax).gt(money(poTax))) {
    throw new TRPCError({
      code: "CONFLICT",
      message: "تراكم ضريبة الاستلام يتجاوز ضريبة أمر الشراء",
    });
  }
  return tax;
}
...
export async function receivePurchase(
  ...
  if (input.payment && input.payment.method !== "CASH") {
    throw new TRPCError({
      code: "BAD_REQUEST",
      message:
        "الدفع غير النقدي للمورد يتطلب سند صرف موثقاً بمرجع الأداة المالية",
    });
  }
...
async function receivePurchaseInTx(
    ...
    ? idempotencyHash({
        ...
        lines: options.autoPostAll
          ...
          : [...input.lines]
              ...
              .sort(
                (left, right) =>
                  left.purchaseOrderItemId - right.purchaseOrderItemId,
              ),
        ...
        shippingPaymentReference:
          input.shippingPaymentReference?.trim() || null,
        shippingCardLastFour: input.shippingCardLastFour?.trim() || null,
        shippingBeneficiarySupplierId:
          input.shippingBeneficiarySupplierId ?? null,
        shippingBeneficiaryName: input.shippingBeneficiaryName?.trim() || null,
        shippingEvidenceReference:
          input.shippingEvidenceReference?.trim() || null,
      })
    : null;
  const paymentRequestToken =
    receiveRequestHash?.slice(0, 16) ??
  ...
  if (input.clientRequestId) {
    ...
    if (existingRefId != null) {
      ...
      const replayInputSum = input.lines.reduce(
        (acc, l) => acc + Number(l.receivedBaseQuantity),
        0,
      );
      const replayActualSum = input.lines.reduce((acc, l) => {
        ...
        if (!it) {
          throw new TRPCError({
            code: "CONFLICT",
            message:
              "تعارض idempotency: بنود الاستلام لا تخص أمر الشراء المُسجَّل",
          });
        }
      ...
      if (replayActualSum < replayInputSum) {
        throw new TRPCError({
          code: "CONFLICT",
          message:
            "تعارض idempotency: كميات الاستلام المطلوبة لا تطابق المسجَّل",
        });
      }
      const replayFully = replayItems.every(
        (r) => (r.receivedBaseQuantity ?? 0) >= r.baseQuantity,
      );
      const [replayShippingRequest] = await tx
        ...
        .where(
          eq(
            ...
            `SHIP-${replayPo.poNumber}-${paymentRequestToken}`,
          ),
        )
        .limit(1);
      const [replaySupplierRequest] = await tx
        ...
        .where(
          eq(
            receipts.referenceNumber,
            `PO-PAY-${replayPo.poNumber}-${paymentRequestToken}`,
          ),
        )
        .limit(1);
      return {
        ...
        shippingPaymentRequestReceiptId: replayShippingRequest
          ? Number(replayShippingRequest.id)
          : null,
        supplierPaymentRequestReceiptId: replaySupplierRequest
          ? Number(replaySupplierRequest.id)
          : null,
  ...
  const shippingBeneficiarySupplierId =
    input.shippingBeneficiarySupplierId ?? null;
  ...
  const isPlaceholder = (value: string) =>
    placeholders.has(value.toLocaleLowerCase("ar-IQ"));
  const freeShippingBeneficiary =
    !rawShippingBeneficiary || isPlaceholder(rawShippingBeneficiary)
      ? ""
      : rawShippingBeneficiary;
  ...
  if (Number(po.branchId) !== Number(poPreview.branchId)) {
    throw new TRPCError({
      code: "CONFLICT",
      message: "تغيّر فرع أمر الشراء أثناء الاستلام؛ أعد المحاولة",
    });
  }
  ...
  if (!allowedStatuses.includes(po.status as "DRAFT" | "SENT" | "CONFIRMED")) {
      ...
      if (racedRefId === input.purchaseOrderId) {
        const [replayShippingRequest] = await tx
          ...
          .where(
            eq(
              receipts.referenceNumber,
              `SHIP-${po.poNumber}-${paymentRequestToken}`,
            ),
          )
          .limit(1);
        const [replaySupplierRequest] = await tx
          ...
          .where(
            eq(
              receipts.referenceNumber,
              `PO-PAY-${po.poNumber}-${paymentRequestToken}`,
            ),
          )
          .limit(1);
        return {
    ...
    throw new TRPCError({
      code: "BAD_REQUEST",
      message: "لا تُرحّل إلا فاتورة شراء غير ملغاة ولم تُرحّل سابقاً",
    });
  }
  ...
  if (
    ...
    throw new TRPCError({
      code: "FORBIDDEN",
      message:
        "لا يجوز استلام أمر شراء أنشأته بنفسك — يلزم شخص آخر لاعتماده (فصل المهام).",
    });
  }
    ...
    ? items
        .map((item) => ({
          ...
          receivedBaseQuantity:
            Number(item.baseQuantity) - Number(item.receivedBaseQuantity ?? 0),
        }))
  ...
  if (totalLanded.gt(0) && poSubtotalForLanded.gt(0)) {
    ...
    for (let i = 0; i < ordered.length; i++)
      if (money(ordered[i].total).gt(0)) lastValued = i;
    ...
    for (let i = 0; i < ordered.length; i++) {
      ...
      if (i === lastValued) {
        ...
        const share = round2(
          totalLanded.times(money(it.total)).dividedBy(poSubtotalForLanded),
        );
  ...
  const work = effectiveLines.map((l) => {
    ...
    if (!item)
      throw new TRPCError({
        code: "BAD_REQUEST",
        message: `بند الشراء ${l.purchaseOrderItemId} لا يخص هذا الأمر`,
      });
    if (
      !Number.isInteger(l.receivedBaseQuantity) ||
      l.receivedBaseQuantity <= 0
    ) {
      throw new TRPCError({
        code: "BAD_REQUEST",
        message: "الكمية المستلمة يجب أن تكون صحيحة موجبة",
      });
    }
    ...
    if (alreadyReceived + l.receivedBaseQuantity > item.baseQuantity) {
      throw new TRPCError({
        code: "BAD_REQUEST",
        message: `الكمية المستلمة تتجاوز المطلوب للبند ${l.purchaseOrderItemId}`,
      });
    }
    return { line: l, item };
  });
  ...
  for (const { line } of work) {
    requestedByItem.set(
      ...
      (requestedByItem.get(line.purchaseOrderItemId) ?? 0) +
  ...
  requestedByItem.forEach((requested, itemId) => {
    ...
    if ((item.receivedBaseQuantity ?? 0) + requested > item.baseQuantity) {
      throw new TRPCError({
        code: "BAD_REQUEST",
        message: `إجمالي الكمية المستلمة يتجاوز المطلوب للبند ${itemId}`,
      });
    }
  });
  ...
  for (const { line, item } of work) {
    ...
    if (factor.lte(0)) {
      throw new TRPCError({
        code: "CONFLICT",
        message: `معامل الوحدة المخزّن لبند الشراء ${item.id} غير صالح`,
      });
    }
    unitFactorMap.set(Number(item.id), factor);
    if (
      factor.gt(1) &&
      !new Decimal(line.receivedBaseQuantity).modulo(factor).isZero()
    ) {
      throw new TRPCError({
        code: "BAD_REQUEST",
        message: `الكمية المستلَمة (${line.receivedBaseQuantity}) غير قابلة للقسمة على معامل الوحدة الأصلي (${factor.toString()})`,
      });
    }
  }
  ...
  const stockMap = new Map(
    stockRows.map((s) => [Number(s.variantId), s.totalQty]),
  );
  ...
  for (const { line, item } of work) {
    ...
    const costPerBase = round2(
      money(item.unitPrice).dividedBy(factor.lte(0) ? new Decimal(1) : factor),
    );
    ...
    const existingQty = Decimal.max(
      new Decimal(stockMap.get(Number(item.variantId)) ?? "0"),
      0,
    );
    ...
    const newCost =
      denom.lte(0) || oldCost.lte(0)
        ...
        : round2(
            existingQty
              .times(oldCost)
              .plus(recvQty.times(capCostPerBase))
              .dividedBy(denom),
          );
    ...
    await tx
      ...
      .set({
        receivedBaseQuantity:
          (item.receivedBaseQuantity ?? 0) + line.receivedBaseQuantity,
      })
    ...
    const isLastReceive =
      priorQty + line.receivedBaseQuantity === item.baseQuantity;
    let lineNet: Decimal;
    if (isLastReceive) {
      lineNet = round2(money(item.total).minus(priorReceivedNet));
    } else {
      const portion = new Decimal(line.receivedBaseQuantity).dividedBy(
        item.baseQuantity,
      );
    ...
    if (po.agreedCurrency === "USD" && item.usdTotal != null) {
      ...
      const lineUsd = isLastReceive
        ...
        : round2(
            money(item.usdTotal).times(
              new Decimal(line.receivedBaseQuantity).dividedBy(
                item.baseQuantity,
              ),
            ),
          );
    ...
    const cumLanded = (k: number): Decimal =>
      k >= item.baseQuantity
        ? lineLanded
        : round2(lineLanded.times(k).dividedBy(item.baseQuantity));
    receivedLanded = receivedLanded.plus(
      cumLanded(priorQty + line.receivedBaseQuantity).minus(
        cumLanded(priorQty),
      ),
    );
  }
  ...
  const rate = poSubtotal.gt(0)
    ? money(po.taxAmount).dividedBy(poSubtotal)
    : new Decimal(0);
  ...
  const priorTaxRow = (
    await tx
      .select({
        v: sql<string>`COALESCE(SUM(${accountingEntries.taxAmount}), 0)`,
      })
  ...
  const receivedTax = cumulativePurchaseTax(
    String(po.taxAmount),
    priorTax.toFixed(2),
    receivedNet,
    rate,
    ...
    const shipMethod = input.shippingPaymentMethod ?? "CASH";
    {
      ...
      if (shippingBeneficiarySupplierId != null) {
        ...
        if (!beneficiary || beneficiary.isActive === false) {
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "جهة الشحن المسجّلة غير موجودة أو غير نشطة",
          });
        }
      ...
      const obligation = await createAccrualObligationTx(tx, {
        ...
        beneficiaryType:
          shippingBeneficiarySupplierId == null ? "OTHER" : "SUPPLIER",
        ...
        actor,
        {
          ...
          beneficiaryId:
            obligation.beneficiarySupplierId == null
              ? null
              : Number(obligation.beneficiarySupplierId),
  ...
  if (
    automaticCashSettlement &&
    input.payment &&
    !explicitPaidNow.eq(receivedTotal)
  ) {
    throw new TRPCError({
      code: "BAD_REQUEST",
  ...
  if (paidNow.gt(0)) {
    if (po.agreedCurrency === "USD") {
      throw new TRPCError({
        code: "BAD_REQUEST",
        message:
          "الفاتورة الدولارية تُسدَّد من شاشة الصيرفة/التسديد لتسجيل مبلغ الدولار وسعر الصرف الفعلي",
      });
    }
    ...
    const poPayable = await purchaseOrderPayableBalanceTx(
      tx,
      input.purchaseOrderId,
    );
    const pendingReserved = await pendingPurchaseSupplierPaymentsTx(
      tx,
      String(po.poNumber),
    );
    ...
    if (!treasuryPrelocked) {
      throw new TRPCError({
        ...
        message: "لم يُقفل مصدر طلب دفعة المورد النقدية",
      });
    }
    const paymentRequest = await createSystemPaymentRequestTx(
      tx,
      {
        branchId: Number(po.branchId),
