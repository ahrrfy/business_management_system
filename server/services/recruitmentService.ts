...
function vacancyScopeCondition(scope: CompanyBranchScope) {
  return scope.branchId == null
    ? undefined
    : eq(jobVacancies.branchId, scope.branchId);
}
...
function vacancyByIdCondition(id: number, scope: CompanyBranchScope) {
  ...
  return branch
    ? and(eq(jobVacancies.id, id), branch)
    : eq(jobVacancies.id, id);
}

async function lockApplicantInScope(
  tx: Tx,
  id: number,
  ...
  if (scope.branchId == null) {
    const [applicant] = await tx
      ...
      .limit(1);
    if (!applicant)
      throw new TRPCError({ code: "NOT_FOUND", message: "المتقدّم غير موجود" });
    return;
  }
  ...
  const [applicant] = await tx
    ...
    .from(jobApplicants)
    .where(
      and(eq(jobApplicants.id, id), eq(jobApplicants.branchId, scope.branchId)),
    )
    .for("update")
    .limit(1);
  if (!applicant)
...
export async function listApplicants(
  ...
  if (filters?.stage)
    conds.push(eq(jobApplicants.stage, filters.stage as never));
  if (filters?.source) conds.push(eq(jobApplicants.source, filters.source));
  if (filters?.vacancyId)
    conds.push(eq(jobApplicants.vacancyId, filters.vacancyId));
  if (filters?.q) {
  ...
  return db
    .select({
      ...getTableColumns(jobApplicants),
      cvFileKey: jobApplicantCvFiles.publicKey,
    })
    .from(jobApplicants)
    .leftJoin(jobVacancies, eq(jobApplicants.vacancyId, jobVacancies.id))
    .leftJoin(
      jobApplicantCvFiles,
      eq(jobApplicantCvFiles.applicantId, jobApplicants.id),
    )
    .where(where)
...
export async function getApplicant(
  ...
  const [a] = await db
    ...
    .where(
      branch ? and(eq(jobApplicants.id, id), branch) : eq(jobApplicants.id, id),
    )
    .limit(1);
  return a ?? null;
}
...
export async function createApplicant(
  ...
  if (input.vacancyId != null) {
    ...
    if (v && (scope || (v.isPublished && hasPublicVacancyDetails(v)))) {
      vacancyId = v.id;
      branchId = v.branchId;
  ...
  const applicantId = await db.transaction(async (tx) => {
    const [res] = await tx.insert(jobApplicants).values({
      ...
      rating:
        input.rating != null
          ? Math.max(0, Math.min(5, Math.trunc(input.rating)))
          : 0,
...
function hasPublicVacancyDetails(
  ...
  return (
    (vacancy.description?.trim().length ?? 0) >=
      MIN_PUBLIC_VACANCY_DESCRIPTION_CHARS &&
...
function assertPublicVacancyDetails(
  vacancy: Pick<VacancyInput, "description" | "requirements">,
): void {
  if (hasPublicVacancyDetails(vacancy)) return;
  throw new TRPCError({
    code: "BAD_REQUEST",
    message: appErrorMessage({
      what: "لا يمكن نشر الوظيفة",
      why: `يلزم وصف تفصيلي لا يقل عن ${MIN_PUBLIC_VACANCY_DESCRIPTION_CHARS} حرفاً ومتطلبات أساسية لا تقل عن ${MIN_PUBLIC_VACANCY_REQUIREMENTS_CHARS} أحرف`,
      doThis: "أضف مهام الوظيفة ومتطلباتها بوضوح ثم احفظ الإعلان من جديد",
    }),
  });
}
...
export async function listOpenVacancies() {
  ...
  const vacancies = await db
    ...
    .orderBy(
      asc(jobVacancies.sortOrder),
      desc(jobVacancies.createdAt),
      desc(jobVacancies.id),
    );
  return vacancies.filter(hasPublicVacancyDetails);
}
...
export async function vacancyApplicantCounts(
  scope: CompanyBranchScope = COMPANY_SCOPE,
...
function normalizeVacancy(input: VacancyInput) {
  ...
  return {
    ...
    openings:
      input.openings != null ? Math.max(1, Math.trunc(input.openings)) : 1,
...
export async function createVacancy(
  ...
  const vacancy = normalizeVacancy({ ...input, branchId });
  if (vacancy.isPublished) assertPublicVacancyDetails(vacancy);
  const [res] = await db.insert(jobVacancies).values(vacancy);
...
export async function setVacancyPublished(
  ...
  if (!existing)
    throw new TRPCError({ code: "NOT_FOUND", message: "الوظيفة غير موجودة" });
  if (isPublished) assertPublicVacancyDetails(existing);
  await db
    .update(jobVacancies)
    .set({ isPublished })
    .where(vacancyByIdCondition(id, scope));
