...
async function seedBase() {
  ...
  await d
    ...
    .values({ id: 1, productId: 1, sku: "PEN-1", costPrice: "0.00" });
  await d
...
describe("purchaseService — receivedNet (last-installment correction = صفر انجراف)", () => {
  it("ثلاثة استلامات بـ١ قطعة لمنتج بـ100 IQD ⇒ AP يساوي 100.00 بالضبط", async () => {
    const po = await createPurchaseOrder(
      {
        ...
        branchId: 1,
        items: [
          { variantId: 1, productUnitId: 1, quantity: "3", unitPrice: "33.34" },
        ],
      },
      actor,
    );
    ...
    await db()
      .update(s.purchaseOrderItems)
      .set({ total: "100.00" })
      .where(eq(s.purchaseOrderItems.id, Number(it.id)));
    await db()
      .update(s.purchaseOrders)
      .set({ subtotal: "100.00", total: "100.00" })
    ...
    await receivePurchase(
      {
        purchaseOrderId: po.purchaseOrderId,
        lines: [
          { purchaseOrderItemId: Number(it.id), receivedBaseQuantity: 1 },
        ],
      },
      actor,
    );
