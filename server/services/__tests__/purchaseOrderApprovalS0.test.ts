  ...
  return createPurchaseOrder(
    {
      ...
      clientRequestId,
      items: [
        { variantId: 1, productUnitId: 1, quantity: "1", unitPrice: "10.00" },
      ],
    },
    creator,
  );
...
describe("S0 — دورة اعتماد أمر الشراء", () => {
  ...
  it("يفرض maker-checker على المنشئ وآخر محرر ثم يسمح لمستخدم مستقل", async () => {
    ...
    const firstRequest = await confirmPurchaseOrder(
      approval(first.purchaseOrderId, first.version, "maker-created-confirm"),
      creator,
    );
    await expect(
    ...
    await updatePurchaseOrder(
      {
        ...
        items: [
          { variantId: 1, productUnitId: 1, quantity: "2", unitPrice: "10.00" },
        ],
      },
      editor,
    );
    const secondRequest = await confirmPurchaseOrder(
      approval(
        second.purchaseOrderId,
        await currentVersion(second.purchaseOrderId),
        "maker-edited-confirm",
      ),
      editor,
    );
    await expect(
      decide(secondRequest.requestId, "maker-edited-decision", editor),
    ).rejects.toMatchObject({ code: "FORBIDDEN" });

    await expect(
      decide(secondRequest.requestId, "independent-approval"),
    ).resolves.toMatchObject({ orderStatus: "RECEIVED", idempotent: false });
  });

  it("لا يعتمد بلا إقرار الاستلام الكامل ويبقي الإرسال صفري الأثر", async () => {
    const po = await createDraft("receipt-confirmation-create");
    const request = await confirmPurchaseOrder(
      approval(po.purchaseOrderId, po.version, "receipt-confirmation-submit"),
      creator,
    );
      ...
      decidePurchaseOrderControl(
        {
          ...
          decisionKey: "receipt-confirmation-missing",
          approve: true,
          reason: "راجعت الأمر لكن لم أؤكد وصول كامل الكميات",
        },
        approver,
      ),
    ).rejects.toMatchObject({ code: "BAD_REQUEST" });
  ...
  it("يعيد replay مطابقاً ويمنع تغيير الحمولة أو عبور الفرع", async () => {
    ...
    await expect(confirmPurchaseOrder(input, approver)).resolves.toMatchObject({
      idempotent: false,
    });
    await expect(confirmPurchaseOrder(input, approver)).resolves.toMatchObject({
      idempotent: true,
    ...
    await expect(
      confirmPurchaseOrder(input, otherBranch),
    ).rejects.toMatchObject({ code: "FORBIDDEN" });
  });
  ...
  it("يجعل المعتمد غير قابل للتعديل ويحسم سباق update/confirm بلا بنود وسطية", async () => {
    ...
    const [updateResult, confirmResult] = await Promise.allSettled([
      ...
      confirmPurchaseOrder(
        approval(raced.purchaseOrderId, raced.version, "race-confirm"),
        approver,
      ),
    ]);
    ...
    let [row] = await db()
      .select()
      .from(s.purchaseOrders)
      .where(eq(s.purchaseOrders.id, raced.purchaseOrderId));
    if (row.status !== "RECEIVED") {
      const latest = await confirmPurchaseOrder(
        approval(
          raced.purchaseOrderId,
          Number(row.version),
          "race-final-confirm",
        ),
        editor,
    ...
    const items = await db()
      ...
      .where(eq(s.purchaseOrderItems.purchaseOrderId, raced.purchaseOrderId));
