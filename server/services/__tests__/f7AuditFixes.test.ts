...
async function seed() {
  ...
  await d
    ...
    .values({ id: 1, productId: 1, sku: "PAP", costPrice: "200.00" });
  await d
...
function caller(role: string, branchId: number, id: number) {
  const ctx = {
    ...
    user: { id, role, branchId },
  } as any;
  return appRouter.createCaller(ctx);
}
function callerOverride(
  role: string,
  branchId: number,
  id: number,
...
describe("M2 (تدقيق ٢٥/٧) — purchase.get يحجب تكلفة البنود اتّساقاً مع الرأس", () => {
  it("دورٌ أساسه manager بـreports=NONE (canSeeCost=false): يُحجب الرأس والبنود معاً — كان يكشف البنود", async () => {
    ...
    const got: any = await callerOverride("manager", 1, 1, {
      reports: "NONE",
    }).purchases.get({ purchaseOrderId: po.purchaseOrderId });
    expect(got.total).toBeNull(); // الرأس محجوب
    expect(got.items.length).toBeGreaterThan(0);
    expect(
      got.items.every((it: any) => it.unitPrice === null && it.total === null),
    ).toBe(true); // البنود محجوبة (الإصلاح)

    // admin (canSeeCost=true) يرى التكلفة كاملةً.
    const adminGot: any = await caller("admin", 1, 1).purchases.get({
      purchaseOrderId: po.purchaseOrderId,
    });
...
describe("F7 #2 — IDOR كتابة عبر الفروع في عهدة التوصيل", () => {
  it("كاشير ف١ يسوّي جهةَ ف٢ ⇒ FORBIDDEN (جهة فرعٍ آخر)", async () => {
    ...
    await expect(
      caller("cashier", 1, 4).delivery.settle({
        partyId: 1,
        amount: "100.00",
        clientRequestId: "f7-settle-cross-cashier",
      } as any),
  ...
  it("كاشير ف٢ يسوّي جهةَ ف٢ ⇒ لا يُرفَض بحارس الفرع (قد يفشل لسبب آخر، لا «فرعٍ آخر»)", async () => {
    ...
    try {
      await caller("cashier", 2, 5).delivery.settle({
        partyId: 1,
        amount: "100.00",
        clientRequestId: "f7-settle-own-cashier",
      } as any);
    } catch (e: any) {
  ...
  it("جهة مشتركة (branchId=null) ⇒ أي كاشير يعبُر حارس الفرع", async () => {
    ...
    try {
      await caller("cashier", 1, 4).delivery.settle({
        partyId: 2,
        amount: "100.00",
        clientRequestId: "f7-settle-shared",
      } as any);
    } catch (e: any) {
      expect(String(e?.message)).not.toMatch(/جهة التوصيل تخصّ فرعاً آخر/);
    }
  ...
  it("مدير الفرع لا يعبُر الفروع على التسوية (قرار المالك ١٢/٨)", async () => {
    ...
    await expect(
      caller("manager", 1, 1).delivery.settle({
        partyId: 1,
        amount: "100.00",
        clientRequestId: "f7-settle-cross-manager",
      } as any),
    ).rejects.toThrow(/فرع/);
  });
});
...
describe("F7 #3 — رياضة كشف حساب المورّد تتّزن مع currentBalance (إشارة كل نوع قيد)", () => {
  it("شراء مُستلَم ١٠٠٠ + مرتجع شراء ٢٠٠ ⇒ الرصيد الجاري = currentBalance = 800 (المنطق الجديد)، والقديم كان 1200", async () => {
    ...
    for (const p of stmt.payments) {
      ...
      const signed =
        p.entryType === "RETURN" ? amt : reducesAP ? amt.neg() : amt;
