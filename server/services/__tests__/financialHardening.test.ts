...
describe("تسوية الصندوق — نسب الإيصالات للوردية (إصلاح العجز/الفائض الوهمي)", () => {
    ...
    await returnSale(
      {
        ...
        resolution: {
          ...
          amount: "10.00",
          shiftId,
          reason: "مرتجع نقدي جزئي لاختبار نسبة الإيصال للوردية",
          disposition: "RESTOCK",
        },
      },
      actor,
  ...
  it("دفع تسليم أمر الشغل النقدي يُنسب للوردية المفتوحة", async () => {
    ...
    const wo = await createWorkOrder(
      { branchId: 1, baseVariantId: 1, title: "درع تكريم", salePrice: "50.00" },
      actor,
    );
  ...
  it("دفع فاتورة آجلة (processPayment) يَشتقّ الوردية تلقائياً عند عدم تمريرها", async () => {
    ...
    await processPayment(
      { invoiceId: sale.invoiceId, amount: "10.00", method: "CASH" },
      actor,
    );
...
describe("عزل الفروع (IDOR) — منع الدفع على فاتورة فرع آخر", () => {
  it("processPayment يَرفض فاتورة فرعٍ مغاير عند enforceBranchId", async () => {
    ...
    // M8: الدفع النقدي يَستوجب وردية مفتوحة (لتسوية الصندوق).
    await openShift(1);
  ...
  it("يرفض استرداداً نقدياً لبيعٍ دُفع بالبطاقة", async () => {
    ...
    await db()
      ...
      .set({
        paymentMethod: "CARD",
        cashBucket: null,
        referenceNumber: "HISTORICAL-CARD-1",
      })
      .where(
        and(
      ...
      returnSale(
        {
          ...
          resolution: {
            ...
            amount: "10.00",
            shiftId,
            reason: "محاولة رد نقدي لعملية بطاقة يجب أن تُرفض",
            disposition: "RESTOCK",
          },
        },
        actor,
    ...
    const r = await returnSale(
      {
        ...
        resolution: {
          ...
          amount: "10.00",
          shiftId,
          reason: "مرتجع نقدي صحيح لعملية دُفعت نقدًا",
          disposition: "RESTOCK",
        },
      },
      actor,
...
describe("ميزان المراجعة / التسوية المستقلّة — يوم كامل يتوازن (درج/ذمم/دفتر)", () => {
  it("درج النقد يتوازن عبر كل التدفّقات + الذمم صحيحة + ربح الدفتر = إيراد − تكلفة", async () => {
    ...
    const shiftId = Number(
      (shRes as any)[0]?.insertId ?? (shRes as any).insertId,
    );
    ...
    const creditInv = (
      await db().select().from(s.invoices).where(eq(s.invoices.customerId, 1))
    )[0];
    await processPayment(
      { invoiceId: Number(creditInv.id), amount: "5.00", method: "CASH" },
      actor,
    );
    ...
    const cashItem = (
      await db()
        .select()
        .from(s.invoiceItems)
        .where(eq(s.invoiceItems.invoiceId, cashSale.invoiceId))
    )[0];
    await returnSale(
      {
        ...
        resolution: {
          ...
          amount: "10.00",
          shiftId,
          reason: "مرتجع نقدي ضمن اختبار ميزان اليوم الكامل",
          disposition: "RESTOCK",
        },
      },
      actor,
    ...
    const wo = await createWorkOrder(
      { branchId: 1, baseVariantId: 1, title: "درع", salePrice: "50.00" },
      actor,
    );
    await approveCurrentDesign(wo.workOrderId);
    await startWorkOrder(wo.workOrderId, actor);
    ...
    for (const e of ents) {
      ...
      expect(money(e.profit).eq(money(e.revenue).sub(money(e.cost)))).toBe(
        true,
      );
    }
  });
