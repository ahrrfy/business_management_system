...
async function seed() {
  ...
  await database.insert(schema.users).values([
    ...
    {
      id: 4,
      openId: "hr-scope-manager-none",
      name: "Branchless manager",
      role: "manager",
      branchId: null,
    },
    {
      id: 5,
      openId: "hr-scope-owner",
      name: "Owner",
      role: "manager",
      branchId: 1,
      isOwner: true,
  ...
  await database.insert(schema.jobVacancies).values([
    {
      id: 51,
      title: "Branch one role",
      branchId: 1,
      description: "Detailed branch one work responsibilities",
      requirements: "Relevant skills",
      isPublished: true,
    },
    {
      id: 52,
      title: "Branch two role",
      branchId: 2,
      description: "Detailed branch two work responsibilities",
      requirements: "Relevant skills",
      isPublished: true,
    },
  ...
  await database.insert(schema.jobApplicants).values([
    ...
    {
      id: 64,
      name: "Branch one general applicant",
      branchId: 1,
      vacancyId: null,
    },
  ]);
  ...
  await database.insert(schema.hrFingerprintDevices).values([
    {
      id: 101,
      name: "Branch one device",
      branchId: 1,
      serialNumber: "HR-SCOPE-SN-1",
      enabled: false,
    },
    {
      id: 202,
      name: "Branch two device",
      branchId: 2,
      serialNumber: "HR-SCOPE-SN-2",
      enabled: false,
    },
    {
      id: 303,
...
describe("employee branch isolation", () => {
  it("limits manager reads and writes, derives creates from the principal, and masks cross-branch IDs", async () => {
    ...
    await expect(scoped.employees.get({ id: 22 })).rejects.toMatchObject({
      code: "NOT_FOUND",
    });
    await expect(scoped.employees.list({ branchId: 2 })).rejects.toMatchObject({
      code: "FORBIDDEN",
    });
    await expect(
    ...
    await expect(
      scoped.employees.setStatus({ id: 22, status: "leave" }),
    ).rejects.toMatchObject({ code: "NOT_FOUND" });
    await expect(
      scoped.employees.setStatus({ id: 11, status: "terminated" }),
    ).rejects.toMatchObject({
      code: "BAD_REQUEST",
      message: expect.stringMatching(/مسار إنهاء الخدمة الرسمي/),
    });
    await expect(scoped.employees.delete({ id: 22 })).rejects.toMatchObject({
      code: "NOT_FOUND",
    });
    await expect(
      scoped.employees.update({
    ...
    const created = await scoped.employees.create({
      firstName: "Scoped",
      lastName: "Create",
      payType: "monthly",
    });
  ...
  it("preserves company-wide admin/owner authority and fails closed for a branchless manager", async () => {
    ...
    expect(
      (await admin.employees.list({ includeInactive: true })).rows,
    ).toHaveLength(3);
    expect(
      (await owner.employees.list({ includeInactive: true })).rows,
    ).toHaveLength(3);
    expect((await admin.employees.get({ id: 22 })).id).toBe(22);
    expect((await owner.employees.get({ id: 22 })).id).toBe(22);
    await expect((await caller(4)).employees.list()).rejects.toMatchObject({
      code: "FORBIDDEN",
    });
  });
});
...
describe("recruitment branch isolation", () => {
  it("scopes vacancies, applicants, counts, and every applicant/vacancy mutation", async () => {
    const scoped = await caller(2);
    expect(
      (await scoped.recruitment.vacancyList()).map((row) => row.id),
    ).toEqual([51]);
    expect((await scoped.recruitment.list()).map((row) => row.id)).toEqual([
      64, 61,
    ]);
    expect(await scoped.recruitment.vacancyCounts()).toEqual({ 51: 1 });

    await expect(scoped.recruitment.get({ id: 62 })).rejects.toMatchObject({
      code: "NOT_FOUND",
    });
    await expect(
      scoped.recruitment.updateStage({ id: 62, stage: "review" }),
    ).rejects.toMatchObject({ code: "NOT_FOUND" });
    await expect(
      scoped.recruitment.setRating({ id: 62, rating: 5 }),
    ).rejects.toMatchObject({ code: "NOT_FOUND" });
    await expect(
      scoped.recruitment.vacancyGet({ id: 52 }),
    ).rejects.toMatchObject({ code: "NOT_FOUND" });
    await expect(
      scoped.recruitment.vacancyPublish({ id: 52, isPublished: false }),
    ).rejects.toMatchObject({ code: "NOT_FOUND" });
    await expect(
      scoped.recruitment.vacancyDelete({ id: 52 }),
    ).rejects.toMatchObject({ code: "NOT_FOUND" });
    await expect(
      scoped.recruitment.vacancyUpdate({ id: 51, title: "Move", branchId: 2 }),
    ).rejects.toMatchObject({ code: "FORBIDDEN" });
    await expect(
      scoped.recruitment.vacancyCreate({ title: "Cross branch", branchId: 2 }),
    ).rejects.toMatchObject({ code: "FORBIDDEN" });
    await expect(
      scoped.recruitment.create({ name: "Cross branch", branchId: 2 }),
    ).rejects.toMatchObject({ code: "FORBIDDEN" });
    await expect(
      scoped.recruitment.create({ name: "Cross vacancy", vacancyId: 52 }),
    ).rejects.toMatchObject({ code: "NOT_FOUND" });

    const vacancy = await scoped.recruitment.vacancyCreate({
      title: "Scoped vacancy",
    });
    expect(vacancy?.branchId).toBe(1);
    const applicant = await scoped.recruitment.create({
      name: "Scoped applicant",
      vacancyId: vacancy!.id,
    });
    expect(applicant?.vacancyId).toBe(vacancy!.id);
    expect(applicant?.branchId).toBe(1);
    const general = await scoped.recruitment.create({
      name: "Scoped general applicant",
    });
    expect(general?.branchId).toBe(1);
    expect(general?.vacancyId).toBeNull();
    const publicGeneral = await scoped.recruitment.submit({
      name: "Public general applicant",
      phone: "+9647701234567",
      residentialAddress: "Baghdad — Karrada",
      portfolioUrl: "https://portfolio.example.com/public-general",
    });
    expect(publicGeneral).toMatchObject({
      branchId: null,
      vacancyId: null,
      source: "external",
      residentialAddress: "Baghdad — Karrada",
    ...
    expect(await scoped.recruitment.get({ id: applicant!.id })).toMatchObject({
      branchId: 1,
      vacancyId: null,
    });
  });
  ...
  it("keeps admin/owner company-wide and rejects branchless HR users", async () => {
    ...
    await expect(
      (await caller(4)).recruitment.vacancyList(),
    ).rejects.toMatchObject({ code: "FORBIDDEN" });
  });
  ...
  it("does not expose or accept vacancies without real public details", async () => {
    ...
    await expect(
      admin.recruitment.vacancyPublish({
        id: incomplete!.id,
        isPublished: true,
      }),
    ).rejects.toMatchObject({
      code: "BAD_REQUEST",
      message: expect.stringMatching(/وصف تفصيلي/),
    });
    ...
    await expect(
      anonymous.recruitment.submit({ name: "No phone" }),
    ).rejects.toMatchObject({ code: "BAD_REQUEST" });
    await expect(
      anonymous.recruitment.submit({
        name: "Email is not collected publicly",
        phone: "+9647701234567",
  ...
  it("never returns CV bytes to a manager outside the vacancy branch", async () => {
    expect(
      await getJobApplicantCvByKey(BRANCH_ONE_CV_KEY, { branchId: 1 }),
    ).toMatchObject({
      fileName: "branch-one.pdf",
    });
    expect(
      await getJobApplicantCvByKey(BRANCH_ONE_CV_KEY, { branchId: 2 }),
    ).toBeNull();
    expect(
      await getJobApplicantCvByKey(BRANCH_ONE_CV_KEY, { branchId: null }),
    ).toMatchObject({
      publicKey: BRANCH_ONE_CV_KEY,
    });
  });
...
describe("HR device branch isolation", () => {
  it("scopes device inventory, punches, users, commands, writes, and mappings", async () => {
    ...
    expect(await scoped.hrDevices.migrationStatus()).toEqual({
      total: 1,
      migrated: 0,
      pending: 1,
    });
    expect(
      (await scoped.hrDevices.punchesList({})).rows.map((row) => row.id),
    ).toEqual([1101]);

    await expect(scoped.hrDevices.get({ id: 202 })).rejects.toMatchObject({
      code: "NOT_FOUND",
    });
    await expect(
      scoped.hrDevices.deviceUsers({ deviceId: 202 }),
    ).rejects.toMatchObject({ code: "NOT_FOUND" });
    await expect(
      scoped.hrDevices.commandsList({ deviceId: 202 }),
    ).rejects.toMatchObject({ code: "NOT_FOUND" });
    await expect(
      scoped.hrDevices.employeeLinks({ employeeId: 22 }),
    ).rejects.toMatchObject({ code: "NOT_FOUND" });
    await expect(
      scoped.hrDevices.update({ id: 202, name: "Forbidden", branchId: 2 }),
    ).rejects.toMatchObject({ code: "NOT_FOUND" });
    await expect(
      scoped.hrDevices.update({ id: 101, name: "Move", branchId: 2 }),
    ).rejects.toMatchObject({ code: "FORBIDDEN" });
    await expect(scoped.hrDevices.migrate({ id: 202 })).rejects.toMatchObject({
      code: "NOT_FOUND",
    });
    await expect(
      scoped.hrDevices.deleteDevice({ id: 202 }),
    ).rejects.toMatchObject({ code: "NOT_FOUND" });
    await expect(
      scoped.hrDevices.approveDevice({ id: 303, branchId: 1 }),
    ).rejects.toMatchObject({ code: "NOT_FOUND" });
    await expect(
      scoped.hrDevices.create({ name: "Cross device", branchId: 2 }),
    ).rejects.toMatchObject({ code: "FORBIDDEN" });
    await expect(
      scoped.hrDevices.mapUser({ deviceId: 101, enrollId: 1, employeeId: 22 }),
    ).rejects.toMatchObject({ code: "NOT_FOUND" });
    await expect(
      mapDeviceUserToEmployee(101, 99, 22, null, { branchId: 1 }),
    ).rejects.toMatchObject({ code: "NOT_FOUND" });
    await expect(scoped.hrDevices.processFolds()).rejects.toMatchObject({
      code: "FORBIDDEN",
    });
    ...
    expect(created?.branchId).toBe(1);
    await expect(
      (await caller(1)).hrDevices.mapUser({
        deviceId: 101,
        enrollId: 1,
        employeeId: 22,
      }),
  ...
  it("keeps admin/owner visibility and rejects users with no assigned branch", async () => {
    ...
    await expect((await caller(4)).hrDevices.list()).rejects.toMatchObject({
      code: "FORBIDDEN",
    });
  });
});
