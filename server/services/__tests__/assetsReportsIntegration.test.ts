...
describe("تكامل الأصول↔التقارير (FA-02 P&L + FI-01 كشف المورد)", () => {
  ...
  it("#2/#3 (تدقيق التثبيت): أصل مشطوب (retired) يُستبعَد من ميزانية الأصول ولا يُهلَك بعد الشطب", async () => {
    ...
    await disposeAsset(a!.id, { kind: "retired", date: "2024-06-01", reason: "تلف كامل" }, ACTOR);

    // خسارة الشطب مُعترَف بها في P&L (سلوك قائم).
    const pl = await getProfitAndLoss({ from: "2024-06-01", to: "2024-06-01" });
    const line = pl.current.expenseLines.find((l) => l.key === "ASSET_DISPOSAL_PL");
    expect(line!.amount).toBe("800000.00");

    // #2: الميزانية تستبعد المشطوب — كان يبقى بـNBV (٨٠٠٬٠٠٠) مناقضاً للخسارة أعلاه.
    const pos = await getFinancialPosition();
    expect(Number(pos.fixedAssets)).toBe(0);

    // #3: ترحيل إهلاك شهر لاحق لا يُهلِك المشطوب (لا قيد DEPR له).
    const run = await postMonthlyDepreciation(2024, 12, ACTOR);
    expect(run.assetsPosted).toBe(0);
    const deprEntries = await db().select().from(s.accountingEntries)
      .where(and(eq(s.accountingEntries.entryType, "ADJUST"), eq(s.accountingEntries.dedupeKey, `DEPR:${a!.id}:2024-12`)));
    expect(deprEntries.length).toBe(0);
  });
});
