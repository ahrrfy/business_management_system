...
describe("H5 — role.update يَلتقط oldValue/newValue لخريطة الصلاحيات", () => {
  it("توسعة صلاحية يَكشفها diff السجلّ", async () => {
    ...
    // newValue للإنشاء يَحوي permissions الأوّليّة (M).
    ...
    expect(created_new.permissions).toEqual({ reports: "READ" });
    ...
    // قبل: reports=READ. بعد: reports=FULL ⇒ التوسعة كاشفة.
    expect(oldV.permissions).toEqual({ reports: "READ" });
    expect(newV.permissions).toEqual({ reports: "FULL" });
  });
});
...
describe("shift.close — يَلتقط expectedCash/variance/handover", () => {
  it("سجلّ الإغلاق يَحوي expectedCash + variance + countedCash", async () => {
    ...
    const shiftId = Number((sh as { insertId?: number }).insertId ?? 0);
