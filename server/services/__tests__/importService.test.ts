...
describe("importCustomers", () => {
  ...
  it("يتخطّى الموجود بالهاتف دون كتابة مكرّر", async () => {
    await importCustomers(
      [{ rowNumber: 1, name: "أحمد", phone: "0770111" }],
      {},
      actor,
    );
  ...
  it("مطابقة legacyCode عند إعادة الاستيراد غير حسّاسة للحالة (تخطٍّ لا اصطدام بالقاعدة)", async () => {
    await importCustomers(
      [{ rowNumber: 1, name: "لاتيني", legacyCode: "AB7" }],
      {},
      actor,
    );
    const r = await importCustomers(
  ...
  it("update يحدّث الموجود بالحقول غير الفارغة فقط", async () => {
    ...
    const c = (
      await db()
        .select()
        .from(s.customers)
        .where(eq(s.customers.phone, "0770111"))
        .limit(1)
    )[0];
...
describe("importProducts", () => {
  ...
  it("يرفض تكلفة استيراد شاذة قبل أي كتابة في الكتالوج أو المخزون", async () => {
    const r = await importProducts(
      [baseRow({ costPrice: "16162", price: "2000" })],
      {},
      actor,
    );
    ...
    expect(r.rows[0].message).toMatch(/شذوذ مالي حاجب|أكبر من سعر/);
    expect(await db().select().from(s.products)).toHaveLength(0);
    expect(await db().select().from(s.productVariants)).toHaveLength(0);
...
describe("importCustomers — الرصيد الافتتاحي (§٥.٢)", () => {
  ...
  it("رصيد ≠ 0 بلا عملة ⇒ فشل الصف (لا افتراض IQD صامت)", async () => {
    const r = await importCustomers(
      [{ rowNumber: 1, name: "بلا عملة", openingBalance: "500" }],
      {},
      actor,
    );
    expect(r.committed).toBe(false);
  ...
  it("رصيد صفري بلا عملة يمرّ — بلا قيد OPENING", async () => {
    const r = await importCustomers(
      [{ rowNumber: 1, name: "صفري", openingBalance: "0" }],
      {},
      actor,
    );
    expect(r.committed).toBe(true);
    expect((await db().select().from(s.customers))[0].currentBalance).toBe(
      "0.00",
    );
  ...
  it("مطابقة legacyCode تسبق الهاتف: تعديل هاتف لا يولّد طرفاً مزدوجاً برصيد", async () => {
    ...
    await db()
      .insert(s.customers)
      .values({ name: "صاحب الهاتف الجديد", phone: "+9647700000002" });
    const r = await importCustomers(
      [
        {
          rowNumber: 1,
  ...
  it("asIs (الافتراض) يُبقي الإشارة كما في الملف", async () => {
    const r = await importSuppliers(
      [
        {
          rowNumber: 1,
          name: "مورد بسالب صريح",
          openingBalance: "-28920682.01",
          currency: "IQD",
        },
      ],
      {},
...
describe("writeErrorMessage — تعريب أخطاء الكتابة (لا رسائل SQL خام للواجهة)", () => {
  ...
  const drizzleErr = (code: string, sqlMessage: string) =>
    Object.assign(
      new Error(
        `Failed query: insert into \`customers\` (...) values (...) params: أحمد,+9647701234567`,
      ),
      {
  ...
  it("اصطدام حارس السباق uq_customer_legacy ⇒ رسالة تعافٍ عربية تذكر القيمة لا القيد", () => {
    const m = writeErrorMessage(
      drizzleErr(
        "ER_DUP_ENTRY",
        "Duplicate entry '118' for key 'customers.uq_customer_legacy'",
      ),
    );
  ...
  it("رسالة drizzle الخام (Failed query) لا تمرّ ولو حوت نصاً عربياً من بيانات الصفوف", () => {
    const m = writeErrorMessage(
      drizzleErr(
        "ER_DATA_TOO_LONG",
        "Data too long for column 'phone' at row 1",
      ),
    );
  ...
  it("ER_DUP_ENTRY عام (غير legacy/باركود) ⇒ رسالة الخريطة العربية العامة", () => {
    const m = writeErrorMessage(
      drizzleErr(
        "ER_DUP_ENTRY",
        "Duplicate entry 'x' for key 'customers.PRIMARY'",
      ),
    );
  ...
  it("خطأ أعمال عربي من خدماتنا (داخل withTx) يمرّ كما هو", () => {
    expect(writeErrorMessage(new Error("المخزون غير كافٍ لإتمام الحركة"))).toBe(
      "المخزون غير كافٍ لإتمام الحركة",
    );
  });
  ...
  it("toArabicMessage: «Failed query» بمعاملات عربية لا يمرّ — يُحال لخريطة MySQL", () => {
    const err = drizzleErr(
      "ER_DUP_ENTRY",
      "Duplicate entry '500' for key 'productUnits.productUnits_barcode_unique'",
    );
    const m = toArabicMessage({
  ...
  it("toArabicMessage: رسالة أعمال عربية حقيقية تمرّ كما هي", () => {
    const m = toArabicMessage({
      ...
      originalMessage: "اسم المنتج مطلوب",
    });
...
describe("مخططات zod (§٥.١)", () => {
  ...
  it("openingStock السالب والكسري يُرفضان في مخطط المنتج", () => {
    ...
    expect(
      productImportRow.safeParse({ ...base, openingStock: -1 }).success,
    ).toBe(false);
    expect(
      productImportRow.safeParse({ ...base, openingStock: 2.5 }).success,
    ).toBe(false);
    expect(
      productImportRow.safeParse({ ...base, openingStock: 0 }).success,
    ).toBe(true);
  });

  it("افتراضات المنتج تُطبَّق في المخطط: تكلفة 0 + وحدة «قطعة» + معامل 1", () => {
    const parsed = productImportRow.parse({
      rowNumber: 1,
      productName: "قلم",
      barcode: "B1",
    });
  ...
  it("customerImportRow يقبل الرصيد الموقَّع ويرفض ٣ منازل وتاريخاً مقلوباً", () => {
    ...
    expect(
      customerImportRow.safeParse({ ...base, openingBalance: "1.234" }).success,
    ).toBe(false);
    expect(
      customerImportRow.safeParse({ ...base, openingBalance: "[123]" }).success,
    ).toBe(false); // الأقواس يحوّلها العميل
    expect(
      customerImportRow.safeParse({ ...base, lastDealtAt: "07-01-2026" })
        .success,
    ).toBe(false);
    expect(
      customerImportRow.safeParse({ ...base, lastDealtAt: "2026-01-07" })
        .success,
    ).toBe(true);
  });
});
...
describe("importProducts — شريحة تكامل الاستيراد (§٥.١/§٥.٣)", () => {
  ...
  const row = (
    over: Partial<ProductImportRow> & { rowNumber?: number },
  ): ProductImportRow =>
  ...
  it("openingStock صفر لا يولّد حركة مخزون", async () => {
    const r = await importProducts(
      [row({ barcode: "B-ZERO", retailPrice: "1000", openingStock: 0 })],
      {},
      actor,
    );
    expect(r.committed).toBe(true);
  ...
  it("صفّان لنفس sku كلاهما بلا تحديد أساس ⇒ فشل برسالة «وحدة أساس واحدة بالضبط»", async () => {
    ...
    expect(r.committed).toBe(false);
    expect(
      r.rows.some((x) => x.message?.includes("وحدة أساس واحدة بالضبط")),
    ).toBe(true);
  });
  ...
  it("إعادة استيراد ملف أصناف (sku=الباركود بالـfallback) ⇒ متجاوَز لا «باركود مُستخدَم» — الاستئناف ممكن", async () => {
    ...
    const rows = [
      row({ barcode: "RERUN-1", retailPrice: "1000", openingStock: 3 }),
    ];
