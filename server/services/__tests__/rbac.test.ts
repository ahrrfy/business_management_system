...
describe("RBAC — الكاشير ممنوع من العمليات الإدارية", () => {
  ...
  it("لا يُنشئ عميلاً", () => expectForbidden(c.customers.create({ name: "x" })));
...
describe("RBAC — أمين المخزن ممنوع من العمليات المالية/الإدارية", () => {
  ...
  it("لا يُنشئ منتجاً", () => expectForbidden(w.catalog.createProduct({ name: "x", variants: [] as never }) as Promise<unknown>));
  it("لا يسحب التقارير المالية", () => expectForbidden(w.reports.apAging()));
  it("لا يُنشئ أمر شراء", () => expectForbidden(w.purchases.createOrder({ supplierId: 1, branchId: 1, items: [] as never })));
});
