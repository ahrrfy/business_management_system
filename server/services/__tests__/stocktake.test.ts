...
import { submitCount, type PortalIdentity } from "../countPortalService";
import {
  approveStocktake,
...
async function approveAllReadyItems(sessionId: number, by = actor) {
  ...
  const ids = review.rows
    ...
    .map((r) => r.variantId);
  if (ids.length)
    await approveStocktakeItems({ sessionId, variantIds: ids }, by);
}
...
describe("الإنشاء واللقطة", () => {
  it("يستبعد الخدمات والكروت والاشتراكات من الجرد ويرفض اختيارها يدوياً", async () => {
    ...
    expect(
      (await getStocktakeRemainingItems(full.sessionId, { q: "كارت زين" }))
        .items,
    ).toHaveLength(0);
    expect(
      (
        await computeStocktakeReview(full.sessionId, { viewerId: actor.userId })
  ...
  it("اللقطة الذرّية: expectedQty من رصيد الفرع وunitCost من تكلفة المتغيّر — ولا تتأثر بتغيير لاحق", async () => {
    ...
    expect(r.code).toMatch(
      new RegExp(`^CNT-${new Date().getFullYear()}-0001-[0-9A-Z]{4}$`),
    );
  ...
  it("ذرّية الإنشاء: تكليف بصنف خارج النطاق ⇒ فشل كامل بلا أي صفّ مكتوب", async () => {
    ...
    const r = await mkSession({
      assignments: [{ name: "عامل أ", method: "PIN", variantIds: [999] }],
    });
    expect(r.itemCount).toBe(3);
  ...
  it("تفرّد الرمز: تسلسل CNT-<السنة>-NNNN-RAND4، وإنشاءان متزامنان لا يتصادمان", async () => {
    ...
    const [r3, r4] = await Promise.all([
      mkSession({ variantIds: [1] }),
      mkSession({ variantIds: [1] }),
    ]);
    const codes = [r1.code, r2.code, r3.code, r4.code];
    expect(new Set(codes).size).toBe(4);
    const all = await db()
      .select({ code: s.stocktakeSessions.code })
  ...
  it("توزيع التكليفات: المُدّعى لصاحبه، وغير المُكلَّف كتلاً متتالية متساوية (±1) بترتيب variantId", async () => {
    ...
    const ownedBy = async (sessionId: number, assignmentId: number) => {
      const items = await db()
        .select()
        .from(s.stocktakeItems)
        .where(eq(s.stocktakeItems.sessionId, sessionId));
      return items
    ...
    expect(await ownedBy(r.sessionId, r.assignments[0].assignmentId)).toEqual([
      1, 2, 3, 4, 5,
    ]);
    ...
    expect(await ownedBy(r2.sessionId, r2.assignments[0].assignmentId)).toEqual(
      [1, 2, 3, 4, 5],
    );
    ...
    expect(await ownedBy(r3.sessionId, r3.assignments[0].assignmentId)).toEqual(
      [1, 2, 3],
    );
  });
});
...
describe("النطاق الحي للجرد الشامل", () => {
  it("يلحق المتغيّرات الجديدة بجلسة FULL قيد العد فقط، ولا يلمس جلسة المراجعة", async () => {
    ...
    await db()
      ...
      .values([
        ...
        {
          id: 7,
          name: "اشتراك رقمي أضيف أثناء الجرد",
          isService: true,
          productType: "DIGITAL_CARD",
        },
      ]);
    ...
    expect(items.find((i) => Number(i.variantId) === 6)?.unitCost).toBe(
      "77.00",
    );
    ...
    await db()
      .insert(s.productVariants)
      .values({ id: 8, productId: 8, sku: "LIVE-8", costPrice: "88.00" });
  ...
  it("الجرد الافتتاحي الحي يستبعد البكج والأمانة وما فُتح مسبقاً", async () => {
    ...
    await db()
      .insert(s.branchStock)
      .values({ variantId: 9, branchId: 1, quantity: 4, openedAt: new Date() });
...
describe("معادلات المراجعة", () => {
  it("معادلة الفرق مع حركات بعد العدّ: بيع OUT بعد countedAt يصحَّح فيطابق (diff=0)", async () => {
    ...
    await insertCount(r.sessionId, 1, r.assignments[0].assignmentId, 20, {
    ...
    const rvRaw = await computeStocktakeReview(r.sessionId, {
      autoAdjust: false,
      viewerId: 1,
    });
...
describe("حارس بصمة إدخال الباركود القديمة — الاعتماد والتوقيع والترحيل", () => {
  ...
  async function openingCount(qty: number) {
    await enableOpeningForIntegrityTest();
    const r = await mkSession({
      sessionType: "OPENING",
      variantIds: [1],
    });
    await insertCount(r.sessionId, 1, r.assignments[0].assignmentId, qty);
    return r;
  }

  async function expectNoFinalizationWrites(sessionId: number) {
    expect(
      await db()
        .select()
        .from(s.inventoryMovements)
        .where(eq(s.inventoryMovements.referenceId, sessionId)),
    ).toHaveLength(0);
    ...
    expect(
      await db()
        ...
        .where(eq(s.stocktakeDecisions.sessionId, sessionId)),
    ).toHaveLength(0);
  }

  it("approveItems يرفض عدّاً يساوي أول 7 أرقام من باركود الوحدة بلا اعتماد أو أثر لاحق", async () => {
    await db()
      .update(s.productUnits)
      .set({ barcode: "6218934005884" })
      .where(eq(s.productUnits.id, 1));
    const r = await openingCount(6_218_934);

    await expectTrpc(
      approveStocktakeItems({ sessionId: r.sessionId, variantIds: [1] }, actor),
      "PRECONDITION_FAILED",
      /باركود|حقل العدد/,
    );
    ...
    expect(item.reviewApprovedBy).toBeNull();
    expect(item.reviewApprovedSnapshotHash).toBeNull();
    expect(
      await db()
        .select()
        .from(s.stocktakeItemReviewEvents)
        .where(eq(s.stocktakeItemReviewEvents.sessionId, r.sessionId)),
    ).toHaveLength(0);
    await expectNoFinalizationWrites(r.sessionId);
  });

  it("approveItems يطبق معامل الوحدة على باركودها البديل قبل مقارنة بصمة التضخم", async () => {
    ...
    const r = await openingCount(1_234_567 * 12);
    ...
    const [item] = await db()
      .select({ approvedAt: s.stocktakeItems.reviewApprovedAt })
      .from(s.stocktakeItems)
      .where(eq(s.stocktakeItems.sessionId, r.sessionId));
  ...
  it("العد القديم بلا attestation يبقى محجوباً حتى لو عُطّلت وحدة الباركود لاحقاً", async () => {
    await db().insert(s.productUnits).values({
      id: 10,
      variantId: 1,
      unitName: "درزن قديم",
  ...
  it("لا يثق بعلامة attestation مزيفة داخل صف عد قديم بلا count operation", async () => {
    await db()
      .update(s.productUnits)
      .set({ barcode: "6218934005884" })
      .where(eq(s.productUnits.id, 1));
  ...
  it("attestation الإدخال الحديث يمنع إنذاراً رجعياً إذا صار الكتالوج يطابق كمية سليمة", async () => {
    const legitimateQty = 77;
    await setStockRow(1, legitimateQty);
    await enableOpeningForIntegrityTest();
    const r = await mkSession({ sessionType: "OPENING", variantIds: [1] });
    ...
    // التغيير اللاحق كان سيجعل الكاشف القديم يقرأ أول سبعة أرقام = 77.
    await db().insert(s.productUnits).values({
      id: 10,
      variantId: 1,
      unitName: "وحدة كتالوج لاحقة",
      conversionFactor: "1",
      isBaseUnit: false,
      isActive: false,
      barcode: "000007712345",
    });

    await expect(
      approveStocktakeItems({ sessionId: r.sessionId, variantIds: [1] }, actor),
    ).resolves.toMatchObject({ ok: true, approvedCount: 1 });
    const [item] = await db()
      .select({ approvedBy: s.stocktakeItems.reviewApprovedBy })
      .from(s.stocktakeItems)
      .where(eq(s.stocktakeItems.sessionId, r.sessionId));
    expect(Number(item.approvedBy)).toBe(actor.userId);
  });

  it("firstSign يعيد فحص سلامة العد حتى في الاستدعاء المتكرر لنفس الموقّع", async () => {
    await db()
      .update(s.productUnits)
      .set({ barcode: "6218934005884" })
      .where(eq(s.productUnits.id, 1));
    const r = await openingCount(77);
    ...
    // يحاكي صف legacy/استيراداً متأخراً وصل بعد التوقيع؛ لا نعتمد على بوابة submit الحديثة.
    await db()
      .update(s.stocktakeCounts)
      .set({ qty: 6_218_934 })
      .where(
        and(
          eq(s.stocktakeCounts.sessionId, r.sessionId),
          eq(s.stocktakeCounts.variantId, 1),
        ),
      );

    await expectTrpc(
      firstSignStocktake(r.sessionId, actor),
      "PRECONDITION_FAILED",
      /باركود|حقل العدد/,
    );
    const [session] = await db()
      ...
      .from(s.stocktakeSessions)
      .where(eq(s.stocktakeSessions.id, r.sessionId));
    expect(Number(session.firstSignBy)).toBe(actor.userId);
    expect(session.approvedBy).toBeNull();
    await expectNoFinalizationWrites(r.sessionId);
  });

  it("finalize يعيد الحساب بعد التوقيع ويوقف أي مخزون أو ledger عند تبدّل العد إلى بصمة barcode", async () => {
    await db()
      .update(s.productUnits)
      .set({ barcode: "6218934005884" })
      .where(eq(s.productUnits.id, 1));
...
describe("حواجز الاعتماد", () => {
  it("إعادة عدّ معلّقة تمنع الاعتماد حتى وصول العدّ الجديد", async () => {
    ...
    await requestStocktakeRecount(
      { sessionId: r.sessionId, variantId: 1, reason: "فرق غير مفهوم" },
      actor,
    );
    const reopened = (
      await db()
    ...
    expect(rv.barriers.canApprove).toBe(false);
    await expectTrpc(
      approveStocktake(r.sessionId, actor),
      "PRECONDITION_FAILED",
      /إعادة العدّ/,
    );
  ...
  it("تعارض عدَّين مفتوح يمنع الاعتماد، والفصل (resolvedPick) يفكّه ويحدّد rawCount", async () => {
    ...
    const [aidA, aidB] = [
      r.assignments[0].assignmentId,
      r.assignments[1].assignmentId,
    ];
    await insertCount(r.sessionId, 1, aidA, 100, { byName: "عامل أ" });
    await insertCount(r.sessionId, 1, aidB, 95, {
      kind: "VERIFY",
      byName: "عامل ب",
      isConflict: true,
    });
    ...
    await resolveStocktakeConflict(
      { sessionId: r.sessionId, variantId: 1, pick: "VERIFY" },
      actor,
    );
    ...
    const r = await createStocktakeSession(
      {
        ...
        variantIds: [1],
        assignments: [
          { name: "المدير يعدّ", method: "USER", userId: actor2.userId },
        ],
      },
      actor, // منشئ = admin
    );
...
describe("الاعتماد الذرّي", () => {
  it("approve كامل: setStock بمرجع STOCKTAKE + قيدا دفتر بقيم صحيحة وdedupeKey فريد + قرارات تلقائية + lastCountedAt + تكرار بلا أثر", async () => {
    ...
    expect(rv.barriers.canApprove).toBe(true);
    expect(rv.ledgerPreview).toEqual({
      shortExpense: "600.00",
      overGain: "400.00",
    });
    ...
    expect(
      mv.every(
        (m) => m.movementType === "ADJUST" && (m.notes ?? "").includes(r.code),
      ),
    ).toBe(true);
    ...
    const over = entries.find(
      (e) => e.dedupeKey === `STOCKTAKE:${r.sessionId}:OVER`,
    )!;
    ...
    const decisions = await db()
      .select()
      .from(s.stocktakeDecisions)
      .where(eq(s.stocktakeDecisions.sessionId, r.sessionId));
    expect(decisions).toHaveLength(5);
    const dm = new Map(decisions.map((d) => [Number(d.variantId), d]));
    expect(dm.get(1)).toMatchObject({
      action: "ADJUST",
      autoApplied: true,
      decidedBy: null,
      finalQty: 104,
    ...
    const bs = await db()
      .select()
      .from(s.branchStock)
      .where(eq(s.branchStock.branchId, 1));
  ...
  it("ROLLBACK كامل عند فشل وسط الاعتماد: لا مخزون ولا حركات ولا قرارات ولا lastCountedAt", async () => {
    ...
    await db()
      .delete(s.accountingEntries)
      .where(
        eq(s.accountingEntries.dedupeKey, `STOCKTAKE:${r.sessionId}:SHORT`),
      );
    const ok = await approveStocktake(r.sessionId, actor);
    expect(ok.ok).toBe(true);
    expect(await stockOf(1)).toBe(99);
  ...
  it("لا إلغاء لجلسة معتمدة؛ وإلغاء جلسة عدّ idempotent بلا أثر مخزوني", async () => {
    ...
    await expectTrpc(
      cancelStocktakeSession({ sessionId: r.sessionId }, actor),
      "BAD_REQUEST",
      /معتمدة/,
    );

    const r2 = await mkSession({ variantIds: [1] });
    await cancelStocktakeSession(
      { sessionId: r2.sessionId, reason: "أُنشئت بالخطأ" },
      actor,
    );
    const again = await cancelStocktakeSession(
    ...
    const sess = (
      await db()
        .select()
        .from(s.stocktakeSessions)
        .where(eq(s.stocktakeSessions.id, r2.sessionId))
    )[0];
...
describe("مخرجات بلا تسريب", () => {
  it("يوحّد القائمة والمتابعة والمتبقي والمراجعة على تقدم واحد حتى مع صف عدّ تاريخي شاذ", async () => {
    ...
    const listed = (await listStocktakeSessions()).find(
      (row) => row.id === r.sessionId,
    )!;
    expect({ total: listed.itemCount, counted: listed.countedCount }).toEqual(
      monitor.progress,
    );
    const remaining = await getStocktakeRemainingItems(r.sessionId, {
      limit: 100,
    });
    ...
    await expectTrpc(
      forceStocktakeReview(incomplete.sessionId, actor),
      "PRECONDITION_FAILED",
      /1\/2/,
    );
  });
  ...
  it("يضيف ويزيل عامل الجرد بأمان: يحفظ العدّات ويبطل الوصول ويعيد توجيه غير المعدود", async () => {
    ...
    const routingAfterAdd = await db()
      ...
      .where(eq(s.stocktakeItems.sessionId, r.sessionId));
    expect(
      new Set(routingAfterAdd.map((row) => Number(row.assignmentId))),
    ).toEqual(new Set([originalId, added.assignmentId]));
    ...
    const oldWorker = (
      await db()
        .select()
        .from(s.stocktakeAssignments)
        .where(eq(s.stocktakeAssignments.id, originalId))
    )[0];
    ...
    const preserved = await db()
      ...
      .where(
        and(
          eq(s.stocktakeCounts.sessionId, r.sessionId),
          eq(s.stocktakeCounts.assignmentId, originalId),
        ),
      );
    expect(preserved).toHaveLength(1);
    const remainingItems = await getStocktakeRemainingItems(r.sessionId, {
      limit: 100,
    });
    expect(remainingItems.items).toHaveLength(2);
    expect(
      remainingItems.items.every(
        (item) => item.assignmentId === added.assignmentId,
      ),
    ).toBe(true);
  ...
  it("يضيف موظفاً بحسابه مرة واحدة ويرفض الإضافة خارج مرحلة العدّ", async () => {
    ...
    await expectTrpc(
      addStocktakeWorker(
        { sessionId: r.sessionId, name: "سالم", method: "USER", userId: 2 },
        actor,
      ),
      "CONFLICT",
      /مضاف بالفعل/,
    ...
    await expectTrpc(
      addStocktakeWorker(
        { sessionId: r.sessionId, name: "عامل لاحق", method: "PIN" },
        actor,
      ),
      "BAD_REQUEST",
      /مرحلة العدّ/,
  ...
  it("كشف المتبقي يعرض بيانات التوجيه فقط ويستبعد ما وصل له عدّ", async () => {
    ...
    const r = await mkSession({ variantIds: [1, 2] });
    const byAlias = await getStocktakeRemainingItems(r.sessionId, {
      q: "BC-PEN-ALT",
      limit: 100,
    });
    ...
    expect(remaining.items[0]).toMatchObject({
      variantId: 2,
      productName: "دفتر 100 ورقة",
      sku: "NB-1",
    });
    expect(JSON.stringify(remaining)).not.toMatch(
      /expectedQty|unitCost|costPrice|bookNow/,
    );
  });
  ...
  it("يعتمد المنتج مرحلياً أثناء COUNTING ويبقي الجلسة والعمال مستمرين", async () => {
    ...
    const approved = await approveStocktakeItems(
      { sessionId: r.sessionId, variantIds: [1] },
      actor,
    );
    ...
    expect(
      review.rows.find((x) => x.variantId === 1)?.reviewApproved?.byName,
    ).toBe("أحمد المدير");
    expect(
      review.rows.find((x) => x.variantId === 1)?.reviewApproved?.isCurrent,
    ).toBe(true);

    // طلب إعادة عدّ يسقط ختم الاعتماد المرحلي لأن أساس المراجعة سيتغير.
    await requestStocktakeRecount(
      { sessionId: r.sessionId, variantId: 1, reason: "تأكيد ميداني جديد" },
      actor,
    );
    const reopenedItem = (
      await db()
    ...
    const events = await db()
      ...
      .where(
        and(
          eq(s.stocktakeItemReviewEvents.sessionId, r.sessionId),
          eq(s.stocktakeItemReviewEvents.variantId, 1),
        ),
      );
  ...
  it("لا يغيّر قرار منتج معتمد بصمت، وإعادة الفتح الصريحة تحفظ السبب وتسمح بالمراجعة", async () => {
    ...
    await approveStocktakeItems(
      { sessionId: r.sessionId, variantIds: [2] },
      actor,
    );
    ...
    await reopenStocktakeItemReview(
      {
        sessionId: r.sessionId,
        variantId: 2,
        reason: "مستند استلام جديد يحتاج تدقيق القرار",
      },
      actor2,
    );
    await decideStocktakeItem(
    ...
    const [item] = await db()
      ...
      .where(
        and(
          eq(s.stocktakeItems.sessionId, r.sessionId),
          eq(s.stocktakeItems.variantId, 2),
        ),
      );
    expect(item.reviewApprovedAt).toBeNull();
    expect(item.reviewReopenReason).toBe(
      "مستند استلام جديد يحتاج تدقيق القرار",
    );
  ...
  it("يعامل اعتماد 0163 بلا بصمة كاعتماد قديم ويعيد تثبيته صراحةً", async () => {
    ...
    const current = await computeStocktakeReview(r.sessionId, {
      viewerId: actor2.userId,
    });
  ...
  it("monitor بالبحث q: recentCounts تصبح المطابقات (اسم/sku) حتى 50 بدل آخر 20، والتهريب يمنع wildcards", async () => {
    ...
    const none = await monitorStocktakeSession(r.sessionId, {
      q: "غير موجود إطلاقاً",
    });
