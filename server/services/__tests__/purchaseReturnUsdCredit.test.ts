...
async function seedBase() {
  ...
  await db()
    .insert(s.products)
    .values({ id: 1, name: "سبايرول حلزوني قياس ٨ ملم" });
  await db()
    .insert(s.productVariants)
    .values({ id: 1, productId: 1, sku: "PR-WHT-8", costPrice: "0.00" });
  ...
  const created = await createPurchaseOrder(
    {
      ...
      taxRatePercent: "0",
      items: [
        { variantId: 1, productUnitId: 2, quantity: "10", unitPrice: "3.5" },
      ],
    },
    actor,
  );
      ...
      lines: [
        {
          ...
          receivedBaseQuantity: Number(item.baseQuantity),
        },
      ],
    },
    actor,
...
describe("مرتجع شراء من أمر دولاري — خصم من ذمة المورد", () => {
  it("يحفظ المستند بتسويته وطريقته، ويعكس الذمّة بالدينار والدولار والمخزون معاً", async () => {
    ...
    const doc = (
      await db()
        ...
        .where(eq(s.purchaseReturns.id, res.purchaseReturnId))
    )[0];
    ...
    const poItem = (
      await db()
        .select()
        .from(s.purchaseOrderItems)
        .where(eq(s.purchaseOrderItems.id, itemId))
    )[0];
      ...
      await db()
        ...
        .where(
          eq(
            ...
            `PURCHASE_RETURN:${res.purchaseReturnId}`,
          ),
        )
    )[0];
  ...
  it("لا يُرجع أكثر من قيمة الفاتورة الدولارية القابلة للإرجاع", async () => {
    ...
    ).rejects.toThrow(/الدولارية تتجاوز/);
  });
});
