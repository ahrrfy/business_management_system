...
/** shift-gate-cash: السندات النقدية تستلزم وردية مفتوحة. helper سريع لفتحها قبل اختبار. */
async function openTestShift(userId = 1, branchId = 1): Promise<number> {
  const r = await db().insert(s.shifts).values({ branchId, userId, openingBalance: "0", status: "OPEN" });
  return Number((r as any)?.[0]?.insertId ?? (r as any)?.insertId);
}
...
describe("reconcileCustomerBalances — يضمّ سندات العميل المستقلّة (إصلاح ٤)", () => {
  beforeEach(async () => { await openTestShift(); });
