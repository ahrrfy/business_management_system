  ...
  await receivePurchase(
    {
      ...
      lines: [{ purchaseOrderItemId: Number(item.id), receivedBaseQuantity }],
    },
    actor,
  );
  return {
...
describe("مرتجع المشتريات المرجعي", () => {
  ...
  it("يثبت معامل وحدة أمر الشراء في العرض والتحقق بعد تعديل معامل الكتالوج", async () => {
    await db()
      .update(s.productUnits)
      .set({ unitName: "درزن", conversionFactor: "12", isBaseUnit: false })
      .where(eq(s.productUnits.id, 1));
    const po = await receivedOrder(10, "0", 120);

    // تعديل الكتالوج بعد إنشاء الأمر واستلامه لا يعيد كتابة لقطة سطر PO.
    await db()
      .update(s.productUnits)
    ...
    expect(new Decimal(resolved.items[0].conversionFactor).equals(12)).toBe(
      true,
    );
    expect(new Decimal(resolved.items[0].remainingQuantity).equals(10)).toBe(
      true,
    );

    // العامل الحي 6 كان سيحوّل 11 إلى 66 أساس ويمرّرها؛ لقطة PO تحوّلها إلى 132 فترفضها.
    await expect(
      createPurchaseReturn(
        {
          clientRequestId: "return-po-unit-snapshot-over",
          supplierId: 1,
          branchId: 1,
          purchaseOrderRefId: po.poId,
          items: [{ purchaseOrderItemId: po.itemId, quantity: "11" }],
          settlement: "CREDIT",
        },
        actor,
      ),
    ...
    const accepted = await createPurchaseReturn(
      {
        clientRequestId: "return-po-unit-snapshot-ok",
        supplierId: 1,
        branchId: 1,
        purchaseOrderRefId: po.poId,
        items: [{ purchaseOrderItemId: po.itemId, quantity: "5" }],
        settlement: "CREDIT",
      },
      actor,
    );
    const returnItem = (
      await db()
        ...
        .where(
          eq(s.purchaseReturnItems.purchaseReturnId, accepted.purchaseReturnId),
        )
    )[0];
    expect(returnItem.baseQuantity).toBe(60);

    const after = await resolveReturnablePurchaseOrder({
      branchId: 1,
      reference: po.poNumber,
    });
    expect(new Decimal(after.items[0].remainingQuantity).equals(5)).toBe(true);
  });
  ...
  it("يعيد المستند نفسه عند replay بالمفتاح والحمولة نفسيهما", async () => {
    ...
    const [first, second] = await Promise.all([
      createPurchaseReturn(input, actor),
      createPurchaseReturn(input, actor),
    ]);
  ...
  it("يمنع over-return متزامناً حتى مع وجود مخزون إضافي من مصدر آخر", async () => {
    ...
    await db()
      ...
      .set({ quantity: sql`${s.branchStock.quantity} + 100` })
      .where(
        and(eq(s.branchStock.branchId, 1), eq(s.branchStock.variantId, 1)),
      );
    const outcomes = await Promise.allSettled([
      createPurchaseReturn(
        {
          clientRequestId: "return-race-a",
          supplierId: 1,
          branchId: 1,
          purchaseOrderRefId: po.poId,
          items: [{ purchaseOrderItemId: po.itemId, quantity: "60" }],
          settlement: "CREDIT",
        },
        actor,
      ),
