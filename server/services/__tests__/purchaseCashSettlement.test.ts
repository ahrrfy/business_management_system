...
async function seed() {
  ...
  await db()
    .insert(s.suppliers)
    .values({ id: 1, name: "مورد نقدي", currentBalance: "0.00" });
  ...
  await db()
    ...
    .values({ id: 1, productId: 1, sku: "PAPER-1", costPrice: "0.00" });
...
async function expectedDrawerCash() {
  return db().transaction(async (tx) =>
    (await computeExpectedCash(tx, 1, "250.00")).toFixed(2),
  );
}
...
describe("أمر الشراء النقدي — مسار المال الكامل", () => {
  it("DRAFT → اعتماد → استلام → طلب صرف → اعتماد: لا ذمة ولا دينار يضيعان", async () => {
    ...
    const [supplierBeforeApproval] = await db()
      .select()
      .from(s.suppliers)
    ...
    const [supplierAfterApproval] = await db()
      .select()
      .from(s.suppliers)
      .where(eq(s.suppliers.id, 1));
    [order] = await db()
  ...
  it("يرحّل SHADOW المخزون مقابل تسوية نقدية ثم يطفئها مقابل الخزينة بلا AP", async () => {
    ...
    expect(lines).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          profile: "PURCHASE_INVENTORY_CASH_CLEARING",
          role: "INVENTORY",
          debit: "1000.00",
          credit: "0.00",
        }),
    ...
    expect(lines).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          profile: "PAYMENT_OUT_PURCHASE_CASH_CLEARING",
          role: "OTHER_LIABILITY",
          debit: "1000.00",
          credit: "0.00",
        }),
  ...
  it("يبقي قيد CASH التاريخي غير الموسوم على AP حتى تُعتمد دفعته القديمة بأمان", async () => {
    ...
    const entries = await db()
      ...
      .where(eq(s.accountingEntries.purchaseOrderId, created.purchaseOrderId))
      .orderBy(asc(s.accountingEntries.id));
    expect(entries.map((entry) => entry.purchaseLiabilityAccount)).toEqual([
      null,
      "AP",
    ]);
    expect(
    ...
    const first = await receivePurchase(
      {
        purchaseOrderId: created.purchaseOrderId,
        lines: [
          { purchaseOrderItemId: Number(item.id), receivedBaseQuantity: 4 },
        ],
    ...
    const second = await receivePurchase(
      {
        purchaseOrderId: created.purchaseOrderId,
        lines: [
          { purchaseOrderItemId: Number(item.id), receivedBaseQuantity: 6 },
        ],
  ...
  it("مرتجع الشراء النقدي واسترداده يعكسان التسوية والمخزون والدرج بلا لمس ذمة المورد", async () => {
    ...
    expect(
      entries.every(
        (entry) => entry.purchaseLiabilityAccount === "CASH_CLEARING",
      ),
    ).toBe(true);
  });
  ...
  it("رفض دفعة PO ثم إعادة تقديمها وإلغاؤها وإعادة طلبها يحافظ على التسوية والتخصيص والنقد بلا ذمة مورد", async () => {
    ...
    const linkedEntries = await db()
      ...
      .from(s.accountingEntries)
      .where(eq(s.accountingEntries.purchaseOrderId, created.purchaseOrderId));
    expect(linkedEntries.map((entry) => entry.entryType)).toEqual([
      "PURCHASE",
      "PAYMENT_OUT",
      "PAYMENT_IN",
    ]);
  ...
  it("يستخدم معامل الوحدة المثبت في سطر PO حتى لو عُدّلت الوحدة قبل الاستلام", async () => {
    ...
    const created = await createApprovedPurchaseOrder({
      ...
      items: [
        { variantId: 1, productUnitId: 1, quantity: "2", unitPrice: "120.00" },
      ],
    });
    await db()
      .update(s.productUnits)
      .set({ conversionFactor: "24" })
      .where(eq(s.productUnits.id, 1));
    const [item] = await db()
      .select()
      .from(s.purchaseOrderItems)
    ...
    await receivePurchase(
      {
        purchaseOrderId: created.purchaseOrderId,
        lines: [
          { purchaseOrderItemId: Number(item.id), receivedBaseQuantity: 12 },
        ],
      ...
      receivePurchase(
        {
          ...
          lines: [
            { purchaseOrderItemId: Number(item.id), receivedBaseQuantity: 5 },
          ],
    ...
    expect(
      (
        await db()
          .select()
          .from(s.purchaseOrderItems)
          .where(eq(s.purchaseOrderItems.id, Number(item.id)))
      )[0].receivedBaseQuantity,
    ).toBe(0);
    expect(
      (await db().select().from(s.suppliers).where(eq(s.suppliers.id, 1)))[0]
        .currentBalance,
  ...
  it("تفصيل AP يستبعد غير المستلم ويعرض الاستلام الجزئي بتاريخ الاعتراف لا تاريخ الطلب", async () => {
    ...
    expect(
      (await getArApAgingDetail({ side: "AP", branchId: 1 })).rows,
    ).toHaveLength(0);

    const oldOrderDate = new Date(Date.now() - 120 * 86_400_000);
    await db()
      .update(s.purchaseOrders)
      .set({ orderDate: oldOrderDate })
      .where(eq(s.purchaseOrders.id, created.purchaseOrderId));
    const [item] = await db()
      .select()
      .from(s.purchaseOrderItems)
  ...
  it("فلتر فرع AP يعيد رصيد الفرع من GL ولا يسرّب رصيد المورد العالمي", async () => {
    await db()
      .insert(s.branches)
      .values({ id: 2, name: "فرع ثانٍ", code: "B2", type: "SALES" });
    await db()
      .update(s.suppliers)
      .set({ currentBalance: "300.00" })
      .where(eq(s.suppliers.id, 1));
    await db()
  ...
  it("طلب D1 المعتمد D2 يظهر في تقارير النقد يوم الاعتماد فقط", async () => {
    const created = await createApprovedPurchaseOrder({
      ...
      settlementType: "CASH",
      items: [
        { variantId: 1, productUnitId: 1, quantity: "1", unitPrice: "1000.00" },
      ],
    });
    const [item] = await db()
      .select()
    ...
    const received = await receivePurchase(
      {
        purchaseOrderId: created.purchaseOrderId,
        lines: [
          { purchaseOrderItemId: Number(item.id), receivedBaseQuantity: 1 },
        ],
    ...
    expect(series.find((point) => point.day === yesterday)?.outflow).toBe(
      "0.00",
    );
    expect(series.find((point) => point.day === today)?.outflow).toBe(
      "1000.00",
    );
    expect(
      (
        await getTreasurySummary({
          from: yesterday,
          to: yesterday,
          branchId: 1,
        })
    ...
    expect(
      (await getTreasurySummary({ from: today, to: today, branchId: 1 }))
        .totalOut,
    ).toBe("1000.00");
    ...
    expect(movements.some((movement) => movement.rawId === requestId)).toBe(
      true,
    );
  });
  ...
  it("مرتجع شراء بسعر يختلف عن WAVG يثبت فرق السعر ولا يكسر قيمة المخزون مقابل AP", async () => {
    ...
    const statement = await getSupplierStatement(1);
    expect(
      statement?.purchaseOrders.find((po) => po.id === highPoId),
    ).toMatchObject({
      total: "2000.00",
      paidAmount: "2000.00",
    });
    ...
    expect(
      pl.expenseLines.find((line) => line.key === "PURCH_RETURN_PRICE_VARIANCE")
        ?.amount,
    ).toBe("-500.00");
      ...
      createPurchaseReturn(
        {
          ...
          clientRequestId: "wavg-high-return",
          items: [
            { purchaseOrderItemId: Number(highPoItem.id), quantity: "5" },
          ],
        },
        receiver,
      ),
