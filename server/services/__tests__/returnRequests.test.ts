...
describe("الاعتماد — فصل المهام واللقطة التفاؤلية", () => {
  it("⭐ مديرٌ آخر يعتمد فيقع المرتجع فعلاً بالمسار القائم", async () => {
    ...
    await returnSale({
      invoiceId, lines,
      refund: { amount: "2000.00", method: "CASH", shiftId: 1 },
      restock: true,
  ...
  it("⭐ فصل المهام: لا يعتمد أحدٌ طلبه بنفسه", async () => {
    ...
    // ومديرٌ آخر يمرّ.
    await expect(loadApprovableRequest(req.requestId, MANAGER_A)).resolves.toBeTruthy();
  });
  ...
  it("الرفض بسببٍ إلزاميّ يُغلق الطلب ويُبقيه مرئياً للموظّف", async () => {
    ...
    await expect(rejectReturnRequest(req.requestId, "لا", MANAGER_A)).rejects.toThrowError(/سبب الرفض/);
