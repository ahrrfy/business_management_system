...
const setStock = (variantId: number, branchId: number, qty: number) =>
  db().insert(s.branchStock).values({ variantId, branchId, quantity: qty });
const count = async (where?: any) =>
  (await db().select().from(s.accountingEntries)).length;
...
async function createGovernedPurchaseReturnSource(unitPrice: string) {
  ...
  if (!invoiceRow || !invoiceLine)
    throw new Error("supplier-invoice source is incomplete");
  ...
  if (!postedInvoice || !allocation)
    throw new Error("posted return source is incomplete");
  return {
    purchaseOrderId: po.purchaseOrderId,
...
describe("#1 idempotency عبر الراوتر الفعلي (النقر المزدوج ⇒ معاملة واحدة)", () => {
  it("returns.create: نفس clientRequestId ⇒ طلب واحد صفري الأثر", async () => {
    ...
    const input = {
      ...
      refund: { amount: "10.00", method: "CASH" as const },
      reason: "مرتجع بطلب رقابي",
      clientRequestId: "ret-key-1",
    };
    await caller().returns.create(input);
    await caller().returns.create(input); // نقرة مزدوجة بنفس المفتاح
    const outReceipts = (await db().select().from(s.receipts)).filter(
      (r) => r.direction === "OUT",
    );
  ...
  it("purchases.receive مغلق وgoodsReceipts.create يعيد نفس الاستلام لنفس clientRequestId", async () => {
    ...
    await expect(
      caller().purchases.receive({
        purchaseOrderId: po.purchaseOrderId,
        lines: [
          { purchaseOrderItemId: Number(poItem.id), receivedBaseQuantity: 5 },
        ],
    ...
    const input = {
      ...
      expectedOrderVersion: Number(approvedOrder.version),
      lines: [
        { purchaseOrderItemId: Number(poItem.id), acceptedBaseQuantity: 5 },
      ],
    ...
    expect(
      (await db().select().from(s.inventoryMovements)).filter(
        (m) => m.movementType === "IN",
      ),
    ).toHaveLength(1);
    expect(
      (await db().select().from(s.accountingEntries)).filter(
        (e) =>
          e.entryType === "ADJUST" &&
          e.dedupeKey === `GRNI:RECEIPT:${first.goodsReceiptId}`,
      ),
    ).toHaveLength(1);
    const sup = (
  ...
  it("vouchers.create: نفس clientRequestId ⇒ سند واحد", async () => {
    ...
    const input = {
      ...
      counterpartyName: "المؤجر",
      description: "إيجار",
      clientRequestId: "vch-key-1",
    };
    ...
    const [pending] = await db()
      .select()
      .from(s.receipts)
      .where(eq(s.receipts.id, Number(r1.receiptId)));
    ...
    await approveVoucher(Number(r1.receiptId), {
      userId: 2,
      branchId: 1,
      role: "manager",
    });
...
describe("#2 عربون أمر الشغل يدخل الصندوق/الدفتر ويُحتسَب عند التسليم", () => {
  it("العربون ⇒ receipt(IN)+shiftId+PAYMENT_IN عند الإنشاء، ويُضمّ لمدفوع الفاتورة عند التسليم", async () => {
    ...
    const depRcpt = (await db().select().from(s.receipts)).find(
      (r) => Number(r.workOrderId) === wo.workOrderId && r.direction === "IN",
    );
    ...
    await deliverWorkOrder(
      {
        ...
        payment: { amount: "15.00", method: "CASH" },
      } as any,
      { ...actor, role: "admin" },
    );
...
describe("#3 تقريب IQD النقدي على الخادم", () => {
  it("بيع نقدي كامل بإجمالي غير مضاعف لـ250 ⇒ يُقرَّب + قيد ADJUST + النقد=المقرّب", async () => {
    ...
    await db()
      .insert(s.productPrices)
      .values([{ productUnitId: 1, priceTier: "WHOLESALE", price: "1240.00" }]);
    // M8: البيع النقدي يَستوجب وردية مفتوحة.
    const { shiftId } = await openShift(
      { branchId: 1, openingBalance: "0" },
      actor,
    ...
    const sale = await createSale(
      {
        ...
        payment: { amount: "1250.00", method: "CASH" },
        cashRoundIQD: true,
      },
      actor,
    );
    ...
    expect(inv.status).toBe("PAID");
    const adj = (await db().select().from(s.accountingEntries)).filter(
      (e) => e.entryType === "ADJUST",
    );
    expect(adj).toHaveLength(1);
    expect(adj[0].amount).toBe("10.00");
    // النقد المستلم (PAYMENT_IN) = الإجمالي المقرّب.
    const pin = (await db().select().from(s.accountingEntries)).filter(
      (e) => e.entryType === "PAYMENT_IN",
    );
...
describe("#4 سعر مرتجع الشراء مشتق من مصدر المطابقة المحكوم", () => {
  it("لا يقبل سعراً من المستدعي ويعيد بالسعر المثبت في تخصيص المطابقة", async () => {
    await db()
      .insert(s.suppliers)
      .values({ id: 1, name: "مورد", currentBalance: "0.00" });
...
describe("#5 ذرّية فتح الوردية", () => {
  it("فتح وردية ثانية لنفس الموظّف/الفرع ⇒ يُرفض", async () => {
    ...
    await expect(
      openShift({ branchId: 1, openingBalance: "0" }, actor),
    ).rejects.toThrow();
  });
  it("بعد الإغلاق يُسمح بفتح وردية جديدة", async () => {
    ...
    await closeShift(
      { shiftId: sh.shiftId, countedCash: "0" },
      { ...actor, role: "admin" },
    );
    await expect(
      openShift({ branchId: 1, openingBalance: "0" }, actor),
...
describe("#2ب استرداد العربون عند إلغاء أمر الشغل (لا نقد عالق)", () => {
  it("إلغاء أمر بعربون مقبوض ⇒ receipt(OUT)+PAYMENT_OUT يعكس PAYMENT_IN (صافي الدفتر صفر)", async () => {
    ...
    const [current] = await db()
      ...
      .from(s.workOrders)
      .where(eq(s.workOrders.id, wo.workOrderId));
    ...
    const inRcpt = rcpts.filter(
      (r) => Number(r.workOrderId) === wo.workOrderId && r.direction === "IN",
    );
    const outRcpt = rcpts.filter(
      (r) => Number(r.workOrderId) === wo.workOrderId && r.direction === "OUT",
    );
    ...
    expect(money(pin[0].amount).minus(money(pout[0].amount)).toFixed(2)).toBe(
      "0.00",
    );
    const wOrder = (
      await db()
    ...
    await receivePurchase(
      {
        purchaseOrderId: po.purchaseOrderId,
        lines: [
          { purchaseOrderItemId: Number(poItem.id), receivedBaseQuantity: 10 },
        ],
      },
      actor,
    );
