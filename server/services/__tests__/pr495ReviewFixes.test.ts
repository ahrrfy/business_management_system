 ...
 *  R4 — تقريب السلّة المختلطة يُشتقّ خادمياً ويطابق المبلغ الذي تثبّته الواجهة.
...
describe("R4 — تقريب السلّة المختلطة مُشتقٌّ خادمياً", () => {
  it("الإجمالي الخادمي يضم فرق تقريب السلّة على الفاتورة الحاملة", async () => {
    const shift = await openReception(2, 1);
    // سلّة مختلطة: بضاعة ١٠٬٠٠٠ + أمر شغل ٥٠٥٠ ⇒ الخام ١٥٬٠٥٠، المقرَّب ١٥٬٠٠٠ (فرق −٥٠).
    const r = await checkoutReception({
      branchId: 1, shiftId: shift.shiftId, customerId: 1,
      paymentMethod: "CASH", paidAmount: "15000.00",
      cashRoundingOverride: "SALE",
      clientRequestId: "r4-round",
      regularSale: { lines: [LINE], amount: "9950.00" },
      workOrders: [{
    ...
    // الفاتورة = أسطرها الخادمية (١٠٬٠٠٠) + فرق تقريب السلّة المشتقّ (−٥٠) = ٩٬٩٥٠.
    expect(String(inv.total)).toBe("9950.00");
    expect(String(inv.paidAmount)).toBe("9950.00");
