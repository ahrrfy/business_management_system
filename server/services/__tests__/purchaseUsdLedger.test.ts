...
import { createPurchaseOrder, receivePurchase, settlePurchaseUsdDirect } from "../purchaseService";
...
async function reset() {
  ...
  for (const t of [
    "idempotencyKeys", "accountingEntries", "exchangeTransactions", "exchangeHouses",
    "receipts", "inventoryMovements", "purchaseOrderItems", "purchaseOrders",
    "branchStock", "productPrices", "productUnits", "productVariants", "products",
    "suppliers", "branches", "users",
...
async function seed() {
  ...
  await d.insert(s.users).values([
    { id: 1, openId: "usd-creator", name: "منشئ", role: "manager", loginMethod: "local", branchId: 1 },
  ...
  await d.insert(s.exchangeHouses).values({
    id: 1, name: "صيرفة الرشيد", balanceIqd: "500000.00", balanceUsd: "0.00", usdCostRate: "0.0000",
  });
}
...
describe("فاتورة المورد الدولارية — التكلفة والذمة والتسديد", () => {
  it("يثبت أسعار البنود بالدولار، يُبقي الشحن خارج الذمّة والتكلفة، ويطفئ الدفعة بسعرها الفعلي", async () => {
    const created = await createPurchaseOrder({
      ...
      usdTotal: "200",
      shippingCost: "100000",
      items: [{ variantId: 1, productUnitId: 1, quantity: "10", unitPrice: "20" }],
    }, creator);

    const po = (await db().select().from(s.purchaseOrders)
      .where(eq(s.purchaseOrders.id, created.purchaseOrderId)))[0];
    ...
    const supplierAfterReceipt = (await db().select().from(s.suppliers)
      .where(eq(s.suppliers.id, 1)))[0];
    const variant = (await db().select().from(s.productVariants)
    ...
    const [supplier, settledPo, house, txn] = await Promise.all([
      ...
      db().select().from(s.purchaseOrders).where(eq(s.purchaseOrders.id, created.purchaseOrderId)).then((r) => r[0]),
      db().select().from(s.exchangeHouses).where(eq(s.exchangeHouses.id, 1)).then((r) => r[0]),
      db().select().from(s.exchangeTransactions).where(eq(s.exchangeTransactions.purchaseOrderId, created.purchaseOrderId)).then((r) => r[0]),
    ]);
    ...
    const [supplierAfterCard, poAfterCard, receipt, entries] = await Promise.all([
      db().select().from(s.suppliers).where(eq(s.suppliers.id, 1)).then((r) => r[0]),
      db().select().from(s.purchaseOrders).where(eq(s.purchaseOrders.id, created.purchaseOrderId)).then((r) => r[0]),
      db().select().from(s.receipts).where(eq(s.receipts.id, direct.receiptId)).then((r) => r[0]),
    ...
    expect(entries.map((e) => e.entryType).sort()).toEqual(["EXCHANGE_FEE", "EXCHANGE_FX_DIFF", "PAYMENT_OUT"]);
