...
//  • قائمة الدخل: `plSnapshot`؛ في ACTIVE مصدرها أدوار journalEntries/journalLines حصراً، وفي
//    OFF/SHADOW تبقى مشتقّةً من الدفاتر التشغيلية مع وسم الأساس صراحةً.
...
export interface MonthlyClosePackResult {
  ...
  profit: {
    revenue: string;
    cost: string;
    /** اسمٌ قديم متوافق: الربح الإجمالي. */
    profit: string;
    grossProfit: string;
...
export async function getMonthlyClosePack(input: MonthlyClosePackInput): Promise<MonthlyClosePackResult> {
  ...
  const empty: MonthlyClosePackResult = {
    ...
    profit: {
      revenue: "0.00", cost: "0.00", profit: "0.00", grossProfit: "0.00",
      totalExpenses: "0.00", netProfit: "0.00",
    },
  ...
  const [salesRow, profitAndLoss, purchases, treasury, arRows, apRows, woRow, returnsRow] = await Promise.all([
    db.execute(sql`
      SELECT
        COUNT(*) AS cnt,
  ...
  return {
    month: input.month,
    period: { from, to },
    accountingBasis: profitAndLoss.accountingBasis,
    sales: {
    ...
    profit: {
      ...
      cost: profitAndLoss.cogs,
      profit: profitAndLoss.grossProfit,
    ...
    expenses: {
      ...
      topCategories: [...profitAndLoss.expenseLines]
