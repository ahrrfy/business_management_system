...
export async function createPurchaseReturn(input: CreatePurchaseReturnInput, actor: Actor) {
  ...
  return withTx(async (tx) => {
    ...
    // قرار المالك: الشحن الوارد **غير مسترد** عند الإرجاع ⇒ يُقيَّد **خسارةً** في الأرباح والخسائر، مع
    // **إبقاء التزامه في ذمّة المورّد** (لا يُصفَّر الرصيد): قيد RETURN يَخصم قيمة البضاعة وحدها من AP،
    // وتبقى حصّةُ الشحن ديناً عليك تُسدَّد لاحقاً نقداً — فتتحقّق الخسارة (المُقيَّدة الآن في P&L).
    ...
    // محصورٌ بالمرتجع المرجعيّ الآجل (CREDIT): الشحن مفهومُ أمرِ شراء، والاسترداد النقديّ مسارٌ مختلف
    // الدلالة (فجوةٌ موازية موثّقة في وصف الـPR — إرجاعٌ نقديٌّ يُخرِج مخزوناً بـWAVG ويسترد قيمة البضاعة).
