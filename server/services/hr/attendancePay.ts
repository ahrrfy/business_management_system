...
export interface AttendancePayInput {
   ...
   * ⚠️ **الحضور الفعليّ يتقدّم عليها**: يومٌ فيه بصماتٌ عملٌ لا إجازة (قرار المالك ١٧/٨).
   */
  unpaidLeaveDates: Set<string>;
  /**
   * أيامٌ **مفتوحة** = ساعاتُها معلومةُ النقص، ولها وجهان (تدقيق ١٧/٨):
   *   • دخولٌ مسجَّل بلا انصراف ⇒ ساعاتٌ **مجهولة** (بصمة ناقصة أو دوامٌ جارٍ)،
   *   • عددُ بصماتٍ فرديّ ⇒ ساعاتٌ **ناقصة** (أزواجُه المكتملة وحدها) وله انصرافٌ مسجَّل.
   * كلاهما **ليس غياباً**: خلطُهما بالغياب كان يدفع الأول صفراً والثاني نصفاً بلا أن يميّزهما
   * أحد — «ما زال في الدوام» و«نسي البصم» و«غاب فعلاً» ثلاثتها بالشكل نفسه. ولذلك لا يُدفع
   * اليوم المفتوح مهما كانت ساعاته المسجَّلة: دفعُ النصف تخمينٌ كدفع الصفر.
   */
