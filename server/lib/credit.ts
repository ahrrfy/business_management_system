...
export async function assertCreditLimit(
  ...
  // ١٥/٩: بطلب من المالك يُستثنى من ذلك العميل الذي لديه رصيد سابق (ذمم عليه أو له رصيد)
  if (limit.isZero()) {
    if (balance.isZero()) {
      throw new TRPCError({
        code: "FORBIDDEN",
        message: "هذا العميل نقديٌّ فقط (حدّ ائتمانه صفر وليس لديه تعامل سابق) — حصّل كامل المبلغ، أو اطلب من المدير رفع حدّه من ملف العميل",
      });
    }
    return; // العميل لديه تعامل سابق، يُسمح بالبيع بالآجل متجاوزاً فحص الصفر
  }
