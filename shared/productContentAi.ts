...
const evidenceKey = z.string().regex(
...
const UNSUPPORTED_MARKETING_TERMS = [
  "الأفضل", "أفضل", "رقم 1", "الأرخص", "مضمون", "أصلي", "فاخر", "مقاوم للماء", "الأكثر مبيعاً", "صديق للبيئة",
];
...
function flattenFacts(facts: ProductFacts): Map<string, string> {
  ...
  Object.entries(facts.attributes).forEach(([key, value]) => out.set(`attributes.${key}`, value));
  ...
  facts.verifiedClaims.forEach((claim, index) => out.set(`verifiedClaims.${index}`, claim));
  return out;
}
...
export function validateAiProductDraft(draft: AiProductDraft, facts: ProductFacts): DraftValidation {
  ...
  for (const [index, claim] of draft.claims.entries()) {
    for (const key of claim.evidenceKeys) {
      if (!factMap.has(key)) blockers.push(`الادعاء رقم ${index + 1} يستخدم دليلاً غير موجود: ${key}`);
    }
  }

  const allText = [draft.seoTitle, draft.shortTitle, draft.posLabel, draft.invoiceLabel, draft.marketingCopy, draft.description, ...draft.keywords].join(" ");
  const knownNumbers = new Set(Array.from(factMap.values()).flatMap(extractNumericTokens));
  for (const token of extractNumericTokens(allText)) {
    if (!knownNumbers.has(token)) blockers.push(`رقم أو قياس غير موجود في حقائق المنتج: ${token}`);
  }
  ...
  for (const term of UNSUPPORTED_MARKETING_TERMS) {
    ...
    if (normalizedText.includes(normalizedTerm) && !approvedClaims.includes(normalizedTerm)) {
  ...
  for (const variant of facts.variants) {
    for (const value of [variant.color, variant.size]) {
      if (value && generalTitles.includes(value)) warnings.push(`قيمة متغير ظهرت في عنوان عام: ${value}`);
    }
  }

  if (draft.description && draft.claims.length === 0) blockers.push("الوصف موجود بلا ادعاءات مرتبطة بأدلة");
  if (draft.marketingCopy && draft.claims.length === 0) blockers.push("النص الترويجي موجود بلا ادعاءات مرتبطة بأدلة");

  return { blockers: Array.from(new Set(blockers)), warnings: Array.from(new Set(warnings)), ok: blockers.length === 0 };
}
