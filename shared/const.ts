...
/**
 * أدوارٌ يُلزَم أصحابها بتفعيل المصادقة الثنائية (2FA) قبل استعمال النظام (قرار المالك ٢٣/٧).
 * الإلزام «فشلٌ آمن»: يُطبَّق فقط إذا كان تشفير 2FA مضبوطاً على الخادم (isCryptoReady) — وإلّا لا إلزام
 * (لئلّا يُقفَل حسابٌ عالي الصلاحية على خادمٍ بلا مفتاح تشفير). الكاشير/البقية اختياريّ كما كان.
 */
export const TWO_FACTOR_REQUIRED_ROLES: readonly string[] = ["admin", "manager"];
