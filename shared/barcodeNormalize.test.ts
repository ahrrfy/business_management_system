...
describe("barcodeIdentityCandidates — صور الهوية التي قد تعيدها محركات المسح", () => {
  ...
  it("يضيف صورةً بلا فراغاتٍ داخلية (ملصقات المتجر «1 0172») ⇒ يطابق الشكلين", () => {
    // المسح يُنتج «1 0172» بمسافة؛ يجب أن يطابق منتجاً مخزّناً «1 0172» أو «10172».
    expect(barcodeIdentityCandidates("1 0172")).toEqual(["1 0172", "10172"]);
    // والإدخال بلا مسافة يبقى صورةً واحدة (المسح هو من يحمل المسافة).
    expect(barcodeIdentityCandidates("10172")).toEqual(["10172"]);
  });

  it("يطوي الأرقام العربية ثمّ يضيف صورةً بلا فراغات (قارئٌ تحت التخطيط العربي)", () => {
    expect(barcodeIdentityCandidates("١ ٠١٧٢")).toEqual(["1 0172", "10172"]);
  });

  it("يبقي المسافة في الصورة الأصلية ولا يكرّر حين لا فراغ داخليّ", () => {
    // Code39 بمسافةٍ معنوية: الأصل يبقى، والصورة بلا فراغات تُضاف كمرشّحٍ إضافيّ لا بديل.
    expect(barcodeIdentityCandidates("AB CD")).toEqual(["AB CD", "ABCD"]);
  });

  it("يطبّق تكافؤ GTIN على الصورة بلا فراغات أيضاً", () => {
    // UPC-A صحيح مكتوبٌ بمسافات ⇒ الصورة بلا فراغات + توسعة الصفر البادئ.
    expect(barcodeIdentityCandidates("0360 0029 1452")).toEqual(["0360 0029 1452", "036000291452", "0036000291452"]);
  });
});
