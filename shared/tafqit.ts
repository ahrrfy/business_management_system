 ...
 * منقول من client/src/lib/printing/tafqit.ts إلى shared/ ليستعمله PDF المستندات الرسمية
 * على الخادم أيضاً (كان القالب الخادمي بلا تفقيط). **نقي بلا اعتماديات** — التقريب هنا
 * HALF_UP على القيمة المطلقة (يطابق سلوك decimal.js في money.ts للمبالغ الموجبة، والمبالغ
 * المفقَّطة موجبة دائماً).
 ...
 * ⚠️ يعمل على الجزء الصحيح فقط. العملة IQD لا فلوس مُتداولة.
 */
...
export function toArabicWords(nRaw: number | string): string {
  const parsed = typeof nRaw === 'string' ? Number(nRaw) : nRaw;
  // Math.round على القيمة المطلقة = HALF_UP (يطابق قاعدة money.ts للمبالغ الموجبة).
  const n = Number.isFinite(parsed) ? Math.round(Math.abs(parsed)) : 0;
  if (n === 0) return 'صفر';
  let rem = n;
  const segs: string[] = [];
