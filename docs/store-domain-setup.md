...
- **استثناء مقصود وضيّق — `/login` و`/my-deliveries` و`/account` مشتركة على المضيفَين ولا تُحوَّل:**
  ...
  تحويلها كان سيقذفه خارج نطاق التطبيق إلى مضيفٍ **بلا جلسة** (الكوكي مقصور بالمضيف بلا `domain`).
  `/account` إلزاميّ في القائمة: كل مستخدم جديد يُنشأ بـ`mustChangePassword` فأول دخول يقوده إلى
  `/account?mustChange=1`. لا يُوسَّع الاستثناء لأي شاشة موظفين أخرى.
- الجذر «/» على الدومين العام: **المندوب المسجَّل يُوجَّه لـ«توصيلاتي» قبل أي شيء** (بداية تطبيقه هي «/»)؛
  الزائر ⇒ المتجر. وموظّف غير مندوب سجّل دخوله من الدومين العام يُنقَل صراحةً لدخول دومين الشركة
  (جلسته هناك لا تُقرأ هنا) — بدل جلسةٍ صامتةٍ عديمة الفائدة.
- رابط الدعوة في بطاقة بيانات الحساب يتبع الدور: **المندوب ⇒ الدومين العام** (دومين تطبيقه)، وبقية
  الموظفين ⇒ دومين الشركة (`CredentialsShare.roleKey`).
...
- `https://alarabiya.online/store-admin` (أو `/pos`، `/reports`…) ⇒ **يُحوَّل تلقائياً** إلى `srv1548487.hstgr.cloud`.
- `https://srv1548487.hstgr.cloud/apply` ⇒ **يُحوَّل** إلى `alarabiya.online/apply`.
- `https://alarabiya.online/login` و`/my-deliveries` و`/account` ⇒ **تبقى** (مسارات تطبيق المناديب المشتركة).
