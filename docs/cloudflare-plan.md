...
- [ ] الجزء أ: تفعيل Cloudflare (المالك — حساب + DNS)
- [ ] الجزء ب: تطبيق طبقة nginx على الخادم (جلسة SSH)
- [ ] الجزء ج: التحقق النهائي

> **الترتيب المرن:** الجزء ب مستقل ويمكن تطبيقه فوراً قبل الجزء أ (ملف realip بلا أثر
> حتى يمرّ الترافيك عبر CF). لكن **لا تفعّل CF قبل تطبيق الجزء ب** — بدون realip ستنقفل
> حدود المعدّل على عناوين حافة CF فيتشارك كل الزوّار ميزانية واحدة.
