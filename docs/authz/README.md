...
يمسح **شجرة الأساس** (origin/<base>) و**شجرة الـhead** ويفشل **فقط على انتهاكٍ يُدخِله الـPR** — بصمةٌ
انتهاكٍ عددُها في head أكبر منه في الأساس. **انتهاكاتُ `main` موجودةٌ في الأساس ⇒ لا تُحسَب**، فلا يفشل
الحارس أبداً على انجراف `main` (المشكلة التي أسقطت الحارس الساكن عند دمج `digitalCardsRouter`). البصمة
مستقلّةٌ عن رقم السطر (`router.name.kind|flags`) فلا يُزيّفها تحرّك الأسطر، ومقارنةُ **العدد** تكشف حتى
مضاعفة انتهاكٍ على مفتاحٍ متصادم. لا أساسَ يُصان ولا `--write-baseline` في المسار المعتاد.
