...
`main`. تحرس **الانحدار** (raw-role/بلا بوّابة/غير مسجَّل مستجدّ) — وهو مستقلٌّ عن اعتماد الكتالوج
النهائيّ (§30.2)؛ إنفاذ الكتالوج الذرّي نفسه يأتي لاحقاً مع المحرك. لإضافةٍ مقصودة تُوسّع الأساس:
راجعها ثم `pnpm authz:baseline` والتزم `authz-baseline.json` (قرار واعٍ موثَّق، لا التفافٌ صامت).

## الأرقام الحاكمة (لقطة 29 يوليو 2026)

| المقياس | القيمة |
|---|---|
| نقاط tRPC | **671** (337 mutation / 334 query) عبر 71 اسمَ راوتر في 72 ملفاً (مسح `server/routers/`) |
