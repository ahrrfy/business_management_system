...
>
> **⚠️ لقطة المراجعة الأصلية = 671 نقطة.** بعد دمج `origin/main` (البطاقات الرقمية) صار المسح الحيّ
> **731 نقطة عبر 72 راوتراً** (انظر README + CSV/JSON للأرقام الحاليّة). أرقام هذا الملف تحليلٌ للقطة
> الأصلية؛ **عدّادات الأعلام أدناه (ADMIN_ONLY 75، RAW_ROLE_GATE 68، LOCAL_GATE 119، HANDLER 5) تبقى
> صحيحة على الشجرة المدموجة** لأنّ نقاط `digital_cards` بوّاباتٌ نظيفة (module-gate/map، صفر انتهاك).

## 1.1 الحصيلة (لقطة المراجعة — 671؛ الحيّ الآن 731)

| السطح | العدد | ملاحظة |
|---|---|---|
| tRPC | **671** (337 mutation، 334 query) — حيّاً **731** بعد الدمج | 72 راوتراً في 73 ملفاً |
| Express | 15 مساراً | تحت 10 نقاط تركيب `/api/*` |
| Webhooks | 5 | whatsapp × 2، instagram × 2، store × 1 |
| مهام مجدولة/دورية | 5 | 2 cron + 3 setInterval |
| **الإجمالي المحصور** | **691** (لقطة) | + المسارات الثابتة و`/healthz` و`.well-known` |
