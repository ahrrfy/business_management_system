...
**الصلاحية:** المدير يُقفِل الشهر متى انتفى الحاجزان. و**المالك له القرار والفصل**: هو وحده يتجاوز حاجزاً قائماً — بسببٍ مكتوبٍ إلزاميّ + قيد تدقيق. يستند إلى `users.isOwner` القائم ([schema.ts:96](../drizzle/schema.ts#L96)) — لا عَلَمَ جديد ولا هجرة.

**الملفات:**
- أنشئ: `server/services/reports/monthCloseReadiness.ts` — `getMonthCloseReadiness({month, branchId})` تُعيد `{ key, label, status: "OK"|"BLOCK"|"WARN", detail }[]` بالتصنيف أعلاه.
- عدّل: `server/routers/reportsRouter.ts` — `monthCloseReadiness` (نفس بوّابة `monthlyClosePack`) + `closeMonth` على **`managerProcedure`** يفوّض إلى `lockPeriod` القائم. **يُعيد فحص الحواجز خادمياً تحت المعاملة** (لا يُصدَّق ادّعاء الواجهة)، و`override: { reason }` يُقبَل **فقط** إن `ctx.user.isOwner` وإلّا `FORBIDDEN`.
- عدّل: `client/src/pages/MonthlyClosePack.tsx` — بطاقة جاهزيةٍ فوق الحزمة + زرّ **«إقفال الشهر»** مُعطَّلٌ ما دام حاجزٌ قائماً؛ ولمن يملك `isOwner` وحده يظهر مسار التجاوز بحقل سببٍ إلزاميّ.
- **بلا هجرة** (`financialPeriods` و`users.isOwner` قائمان).

**اختبارات مُلزِمة:** وردية مفتوحة تحجب · سند معلَّق يحجب · انحراف مطابقة **لا** يحجب (تنبيه) · مديرٌ يتجاوز حاجزاً ⇒ `FORBIDDEN` · مالكٌ يتجاوز بسببٍ ⇒ ينجح ويُكتب قيد تدقيق · تجاوزٌ بلا سبب ⇒ يُرفض · الفحص يُعاد خادمياً (طلبٌ مُلفَّق بحواجز قائمة يُرفض).
