...
pnpm health:check
# الجدولة اليومية 08:00 (بعد سحب 07:30) + إصلاح إعدادات المهمة: انظر رأس scripts/health-check-local.ps1
```
...
> عولج بضبط `StartWhenAvailable=true` + `DisallowStartIfOnBatteries=false` على مهمتَي السحب والفحص
> (السطر الجاهز في رأس كل سكريبت) — طبِّق الضبط نفسه على أي مهمة مجدولة جديدة.
