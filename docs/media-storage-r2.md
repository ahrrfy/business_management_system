...
5. أنشئ المرآة الأولية على جهاز/قرص خارج خادم التطبيق، ثم تحقق منها. الأداة تراكمية: تنسخ وتتحقق
   ...
   محلية ضيقة. `restore-drill` لا يتصل بـR2: ينسخ عينة حقيقية من المرآة إلى وجهة على filesystem
   device آخر؛ اختلاف المجلد أو كونه شقيقاً لا يكفي ويُرفض عند تطابق هوية الجهاز. يعيد التحقق من
   SHA-256، ويسجل هويتي الجهاز وبصمة اسم مضيف التمرين، ثم يكتب `receipt.json` و`receipt.sha256`. تطبع الأداة الأعداد والبصمات
...
ونطاق المستأجر المشتق من `tenantNamespace`. كما يعيد اشتقاق `sourceScopeSha256` من
`R2_ACCOUNT_ID + NUL + R2_IMAGE_BUCKET` الحاليين، فلا تقبل manifest صحيحة تخص bucket آخر. ثم يعيد
قراءة ملفات الاستعادة ويتحقق من اختلاف filesystem device عن جهاز المرآة ومطابقة هويتيهما للإيصال،
والحجم وSHA-256 ومنع symlink/traversal قبل فتح delete. مرآة `single/studio/` لا تفوض شركة، ومرآة
...
### إجراء التفعيل — إنتاج الأدلّة ثم فتح النافذة

الشروط أعلاه كانت موصوفةً بلا إجراءٍ يُنتجها، فبقي الكنس مقفلاً عملياً. الأدلّة كلّها
يولّدها [`scripts/r2-cold-mirror.mjs`](../scripts/r2-cold-mirror.mjs) على **جهاز المكتبة**
لا على الـVPS، وبقرصَين مختلفَين فيزيائياً (الحارس يرفض جهازاً واحداً).

**١) نسخُ المرآة وإنتاج البيان** — يُنتج `manifest.json` وبصمته:

```bash
R2_MIRROR_CONFIRM=RUN_CUMULATIVE_PRIVATE_R2_MIRROR R2_MIRROR_MODE=copy R2_COLD_MIRROR_DIR=/mnt/mirror node scripts/r2-cold-mirror.mjs
```

**٢) تمرين الاستعادة** — على قرصٍ **آخر**؛ يُنتج `receipt.json` ويطبع البصمتين معاً:

```bash
R2_MIRROR_CONFIRM=RUN_CUMULATIVE_PRIVATE_R2_MIRROR R2_MIRROR_MODE=restore-drill R2_COLD_MIRROR_DIR=/mnt/mirror R2_DR_RESTORE_DIR=/mnt/drill node scripts/r2-cold-mirror.mjs
```

**٣) فتح النافذة** بالمتغيرات الستّة أعلاه (البصمتان من مخرَج الخطوة ٢)، ثم `pm2 reload`.
العامل الدوريّ يمسح كل ساعة (`PRODUCT_STUDIO_STAGING_SWEEP_BATCH`، الافتراضي ١٠٠).
راقب `productStudio.staging.swept` في السجلّ.

**٤) أعِد الوضع إلى `audit`** بعد تفريغ المتراكم. إقرار الحذف لا يُترك دائماً.

> ⚠️ **البيان يشيخ خلال ٧ أيام والإيصال خلال ٩٠**؛ نافذةٌ تُترك مفتوحةً ستتوقّف من تلقاء
> نفسها بخطأٍ مغلق — وهذا مقصود، لا عطب.

**كلفة التشغيل — أُصلحت قبل هذا الإجراء:** كان تحميل الإثبات يجري في رأس دالّة الكنس
دائماً (حتى بلا مرشّحٍ للحذف)، والكنس نفسه يُستدعى **بعد كل إرسالٍ ناجح داخل مسار طلب
الموظف** ⇒ إضافةُ غيغابايتات من القراءة والتجزئة إلى كل عملية. صار التحميل كسولاً
ومرّةً واحدة لكل مسح وخارج أيّ معاملة، والكنس حكراً على العامل الدوريّ. وفهارس
`0214` شرطُ جدوى: إثباتُ غياب المرجع كان مسحاً كاملاً لجدولين لكل مرشّح.
