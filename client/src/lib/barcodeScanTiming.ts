...
export class ScanBurstDetector {
  ...
  /**
   * نافذة استعادة الحرف الأوّل — سخيّةٌ جداً عمداً (١٤/٩، بلاغ المالك «B51822572015» و«الطابعي»).
   * بعض القارئات تُرسل الحرف الأوّل بتأخير بدءٍ ملموس (طاقة/تهيئة الجهاز) ثمّ تُطلق البقيّة سريعاً؛
   * فالفاصل بين الحرف الأوّل والثاني قد يبلغ مئات الملّي، بينما تُثبِت الومضةُ اللاحقةُ السريعةُ أنّه
   * مسحٌ حقيقيّ. النافذة الضيّقة (260) كانت تُسقط الحرفَ الأوّل (حرفٌ «B» أو رقمٌ) فيفشل باركودُ المتجر
   * كلّه. توسيعُها لا يمسّ القارئ السريع (لا يستعمل الاستعادة أصلاً — ومضتُه تبدأ من الحرف الثاني)،
   * ولا يزيد الإيجابيّ الكاذب إلّا في حالةِ «يكتب البشرُ حرفاً ثمّ يمسح خلال ≤عتبة الومضة» النادرة
   * التي تُكلّف إعادةَ مسحٍ لا أكثر. الحلّ الأمثل يبقى ضبطَ القارئ (تأخير بين المحارف = 0 + لاحقة Enter).
   */
  private get lookbackMs(): number {
    return Math.max(this.intraGapMs * 8, 1500);
  }
