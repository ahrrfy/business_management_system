...
export class ScanBurstDetector {
  ...
  feed(input: ScannerKeyEvent, now: number): FeedAction {
    ...
    // مسافةٌ خارجَ ومضةٍ نشطة: الإنسان يكتب بفاصل > 40مي (أسرع نقرة إنسانية 60-120مي)، والقارئ
    // الفيزيائي يبثّ بـ 5-15مي. لذا أي مسافة بفاصل > 40مي هي فاصلُ كلماتٍ إنساني (مثل «عمار السلامي»)،
    // نكسر أيّ مرشّحٍ ناشئ ونمرّرها كما تُكتب (pass) فلا يُختطَف بحثُ المستخدم ولا تُبتلع المسافة.
    // أمّا المسافة بسرعة القارئ الحقيقية (≤ 40مي) فتمر للمنطق أدناه فتبقى ملتقطة كرمز باركود («A 1234»).
    if (input.key === " " && gap > 40) {
