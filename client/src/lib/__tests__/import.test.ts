...
describe("integer — نقطة زائدة وسياسة المخزون السالب", () => {
  ...
  it("السالب يُقصّ صفراً مع تحذير (لا فشل)", () => {
    const r = coerceValue(stock, "-7");
    expect(r.value).toBe(0);
    expect(r.error).toBeNull();
    expect(r.warning).toContain("استُورد صفراً");
  });
  it("السالب بأقواس مربعة («[1.]» — ١٤٤١ صفاً مقيسة في ملف الأصناف) يُقصّ صفراً مع تحذير لا فشل الصف", () => {
    for (const v of ["[1.]", "[2.]", "[402.]"]) {
      const r = coerceValue(stock, v);
      expect(r.value, v).toBe(0);
      expect(r.error, v).toBeNull();
      expect(r.warning, v).toContain("استُورد صفراً");
    }
  });
  it("السالب بأقواس محاسبية (7.) يُقصّ صفراً أيضاً، و[0.] يبقى صفراً موجباً بلا تحذير", () => {
    const r = coerceValue(stock, "(7.)");
    expect(r.value).toBe(0);
    expect(r.warning).toContain("استُورد صفراً");
  ...
  it("الكسري السالب يُقصّ لا يفشل ([4.5] و-4.5 — القصّ قبل فحص الصحيح)", () => {
    expect(coerceValue(stock, "[4.5]").value).toBe(0);
    expect(coerceValue(stock, "-4.5").value).toBe(0);
  });
...
describe("buildRows — منتجات: تطبيع الوحدة وفحص sku/باركود", () => {
  ...
  it("المخزون السالب يولّد تحذيراً على الصف لا خطأ", () => {
    const parse: ImportParseResult = {
    ...
    expect(built[0].errors).toHaveLength(0);
    expect(built[0].values.openingStock).toBe(0);
    expect(built[0].warnings.some((w) => w.message.includes("سوِّه من شاشة الجرد"))).toBe(true);
  });
});
