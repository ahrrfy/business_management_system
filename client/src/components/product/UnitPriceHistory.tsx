...
export function UnitPriceHistory({ variantId, unitName, variantLabel, className }: UnitPriceHistoryProps) {
  const [open, setOpen] = useState(false);
  const canOpen = Number.isInteger(Number(variantId)) && Number(variantId) > 0 && !!unitName.trim();
  ...
  return (
    <>
      <button
        ...
        disabled={!canOpen}
        title={canOpen ? "عرض سجل تغيّر سعر هذه الوحدة" : "احفظ الوحدة أولاً لعرض سجل السعر"}
        className={cn(
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-5xl">
          <DialogHeader>
            ...
            <DialogDescription>
              آخر 50 تغييراً مسجّلاً للسعر قبل وبعد، مع المنفّذ والتوقيت والسبب أو موجة التسعير.
              {variantLabel ? <span className="font-medium text-foreground"> المتغيّر: {variantLabel}.</span> : null}
            </DialogDescription>
          </DialogHeader>

          {resolveQ.isLoading ? (
            <div className="py-10 text-center text-sm text-muted-foreground">جارٍ تحديد الوحدة…</div>
          ) : resolveQ.isError ? (
            <ErrorState message={resolveQ.error.message} onRetry={() => resolveQ.refetch()} />
          ) : productUnitId == null ? (
            <div className="rounded-md border border-dashed px-4 py-10 text-center text-sm text-muted-foreground">
              لم تعد هذه الوحدة محفوظة بهذا الاسم. احفظ التعديلات ثم أعد المحاولة.
            </div>
          ) : historyQ.isLoading ? (
            <div className="py-10 text-center text-sm text-muted-foreground">جارٍ تحميل سجل السعر…</div>
          ) : historyQ.isError ? (
            <ErrorState message={historyQ.error.message} onRetry={() => historyQ.refetch()} />
              ...
              <table className="w-full min-w-[780px] text-sm">
                <thead className="sticky top-0 bg-muted/95 text-xs text-muted-foreground backdrop-blur">
                  <tr>
                    ...
                    <th className="px-3 py-2 text-right font-medium">السبب / الموجة</th>
                  </tr>
                </thead>
                <tbody>
                  {(historyQ.data ?? []).map((row) => {
                    ...
                    return (
                      <tr key={row.id} className="border-t align-top">
                        <td className="whitespace-nowrap px-3 py-2 text-xs text-muted-foreground tabular-nums" dir="ltr">
                          {fmtDateTime(row.createdAt)}
                        </td>
                        <td className="px-3 py-2">{display.tier}</td>
                        <td className="px-3 py-2 tabular-nums" dir="ltr">{display.oldPrice}</td>
                        <td className="px-3 py-2 font-semibold tabular-nums" dir="ltr">{display.newPrice}</td>
                        <td className="px-3 py-2">{display.actor}</td>
                        <td className="px-3 py-2">
                          <div>{display.reason}</div>
                          <div className="mt-0.5 text-xs text-muted-foreground">{display.source}</div>
                        </td>
                      </tr>
                    );
                  })}
