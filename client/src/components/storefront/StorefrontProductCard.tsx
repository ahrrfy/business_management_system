...
export function StorefrontProductCard({
  ...
  const productTitle = p.productName ?? p.name ?? "منتج بدون اسم";
    ...
    <article
      className={`store-product-card group relative flex h-full flex-col overflow-hidden rounded-2xl border bg-white transition-all duration-300 dark:bg-slate-900 ${
        showAdded
          ...
          : p.inStock
          ? "border-slate-200/80 hover:-translate-y-1 hover:border-[#0E806A]/40 hover:shadow-md dark:border-slate-800 dark:hover:border-emerald-900"
    ...
    >
      ...
      <div className="relative aspect-[4/3] w-full shrink-0 overflow-hidden bg-slate-50 text-right dark:bg-slate-950">
        <button
          ...
          aria-label={`فتح تفاصيل ${productTitle}`}
        ...
        <div className="pointer-events-none absolute inset-x-2 top-2 z-10 flex items-start justify-between gap-1">
          ...
          {/* أزرار الإجراءات — تلتصق باليسار في RTL وتمنع التزاحم */}
          <div className="pointer-events-auto flex items-center gap-1 shrink-0">
            ...
            {/* زر المشاركة — متاح على الشاشات الكبيرة لتفادي التزاحم على الهواتف */}
            <button
              type="button"
              onClick={() => onShare(p.productId, productTitle)}
              aria-label={`مشاركة ${productTitle}`}
              className={`store-action-button hidden sm:flex !size-8 !min-h-8 !min-w-8 items-center justify-center rounded-full bg-white/95 text-slate-600 shadow-xs ring-1 ring-slate-200/80 backdrop-blur-sm transition-all hover:scale-110 hover:text-emerald-700 active:scale-95 before:absolute before:-inset-1.5 before:content-[''] dark:bg-slate-800/95 dark:text-slate-300 dark:ring-slate-700 ${
      ...
      <div className="flex flex-1 flex-col p-3 sm:p-4">
        ...
        <div className="mt-1 flex min-h-[2.5rem] items-start">
          ...
          >
            <span className="line-clamp-2 text-xs font-bold leading-snug text-[#183D36] transition-colors group-hover:text-[#0E806A] dark:text-slate-100 dark:group-hover:text-emerald-400">
              {productTitle}
            </span>
          </button>
        </div>
        ...
        {!p.isCustomizable && cartQuantity > 0 && onUpdateQuantity ? (
          ...
          <button
            ...
            disabled={!orderable}
            className={`store-primary-action mt-auto flex h-10 sm:h-11 w-full items-center justify-center gap-1.5 rounded-xl px-2 text-xs font-black text-white shadow-xs transition-all duration-200 active:scale-95 disabled:cursor-not-allowed disabled:bg-slate-200 disabled:text-slate-400 disabled:shadow-none dark:disabled:bg-slate-800 dark:disabled:text-slate-600 ${
              showAdded
                ? "bg-emerald-700 shadow-emerald-600/30 scale-[1.02]"
                : "bg-[#0E806A] hover:bg-[#0b6b58] shadow-[#0E806A]/20"
            }`}
          >
            {showAdded ? (
              <>
