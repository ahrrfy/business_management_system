...
export function CheckoutColumn(props: CheckoutProps) {
  return (
    <div style={{ width: 500, flexShrink: 0, display: "flex", flexDirection: "column", gap: 6, minHeight: 0 }}>
...
export function PaymentBlock({
  ...
  const Method = ({ m, Icon, label, disabled = false }: { m: PaymentMethod; Icon: React.ComponentType<{ "aria-hidden"?: boolean; size?: number }>; label: string; disabled?: boolean }) => (
    <button onClick={disabled ? undefined : () => setMethod(m)} disabled={disabled}
      ...
      style={{ flex: 1, height: 32, display: "flex", alignItems: "center", justifyContent: "center", gap: 5, border: `1.5px solid ${method === m ? C.primary : C.border}`, borderRadius: 7, background: method === m ? C.primary : C.card, color: method === m ? C.primaryFg : C.fg, fontWeight: 800, fontSize: 12, cursor: disabled ? "not-allowed" : "pointer", opacity: disabled ? 0.55 : 1, fontFamily: "inherit", touchAction: "manipulation" }}>
      <Icon aria-hidden size={15} />{label}
    </button>
  );

  return (
    <div style={{ flexShrink: 0, display: "flex", flexDirection: "column", background: C.card, borderRadius: 10, border: `1px solid ${C.border}`, overflow: "hidden" }}>
      <div style={{ padding: "5px 12px", background: C.primary, display: "flex", justifyContent: "space-between", alignItems: "center", flexShrink: 0 }}>
        <span style={{ fontSize: 12.5, color: C.primaryFg, fontWeight: 700, opacity: 0.92 }}>الإجمالي</span>
        <div style={{ display: "flex", alignItems: "baseline", gap: 4 }}>
          <span style={{ fontSize: 20, fontWeight: 900, direction: "ltr", letterSpacing: "-0.5px", color: C.primaryFg }}>{fmt(total)}</span>
          <span style={{ fontSize: 11, color: C.primaryFg, opacity: 0.85 }}>د.ع</span>
        </div>
      </div>
      <div style={{ flexShrink: 0, padding: "5px 10px 0" }}>
        <div style={{ background: C.muted, border: `1.5px solid ${C.border}`, borderRadius: 7, padding: "3px 10px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 6, height: 32, marginBottom: 5 }}>
          <span style={{ fontSize: 12, color: C.mutedFg, flexShrink: 0, fontWeight: 700 }}>
            {editingInvoice ? "دفعة إضافية" : "المبلغ المستلم"}
          </span>
          <div style={{ display: "flex", alignItems: "center", gap: 5, flex: 1, justifyContent: "flex-end" }}>
            <button
              ...
              style={{ height: 22, padding: "0 7px", border: `1px solid ${C.primary}`, borderRadius: 5, background: C.primarySoft, color: C.primary, fontFamily: "inherit", fontSize: 11, fontWeight: 800, cursor: cartLen ? "pointer" : "not-allowed", opacity: cartLen ? 1 : 0.5, flexShrink: 0, touchAction: "manipulation" }}
            >
              = الكل
            </button>
            <input
              ...
              style={{ flex: 1, minWidth: 0, maxWidth: 160, border: "none", outline: "none", background: "transparent", fontSize: 16, fontWeight: 900, direction: "ltr", textAlign: "left", fontFamily: "inherit", color: payInput ? (isOwing ? C.amber : C.primary) : C.fg }}
            />
          </div>
        </div>
        ...
        {method !== "CASH" && (
          <div id="print-pos-external-payment-proof" role="status" style={{ marginBottom: 5, display: "flex", alignItems: "flex-start", gap: 4, color: C.mutedFg, fontSize: 11, fontWeight: 700, lineHeight: 1.4 }}>
      ...
      <div style={{ flexShrink: 0, padding: "5px 8px 6px", borderTop: `1px solid ${C.border}` }}>
        <div style={{ minHeight: 18, display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 4 }}>
          {!cartLen && <span style={{ fontSize: 11.5, color: C.mutedFg }}>اختر خدمة للبدء</span>}
          {cartLen > 0 && hasZeroLine && <span style={{ fontSize: 11, color: C.amber, fontWeight: 700, display: "inline-flex", alignItems: "center", gap: 3 }}><Pencil aria-hidden size={10} /> أدخل سعراً للخدمات ذات السعر اليدوي</span>}
          {cartLen > 0 && !hasZeroLine && !payInput && !editingInvoice && <span style={{ fontSize: 11, color: C.mutedFg }}>{method === "CASH" && cashTotal !== total ? `نقداً يُقرَّب إلى ${fmt(cashTotal)} د.ع` : "أدخل المبلغ أو «إتمام» للدفع الكامل أو «حجز»"}</span>}
          {cartLen > 0 && !!payInput && isChange && (<><span style={{ fontSize: 12, color: C.mutedFg, fontWeight: 600 }}>الباقي للعميل</span><span style={{ display: "inline-flex", alignItems: "center", gap: 4 }}><span style={{ fontSize: 17, fontWeight: 900, color: C.success, direction: "ltr" }}>{fmt(change)} <span style={{ fontSize: 10.5, fontWeight: 500, color: C.mutedFg }}>د.ع</span></span><CopyButton value={change} title="نسخ الباقي" successMessage="تم نسخ الباقي" /></span></>)}
          {cartLen > 0 && !!payInput && isOwing && (<><span style={{ fontSize: 12, color: C.amber, fontWeight: 600 }}>المتبقي (آجل)</span><span style={{ display: "inline-flex", alignItems: "center", gap: 4 }}><span style={{ fontSize: 17, fontWeight: 900, color: C.amber, direction: "ltr" }}>{fmt(credit)} <span style={{ fontSize: 10.5, fontWeight: 500 }}>د.ع</span></span><CopyButton value={credit} title="نسخ المتبقي" successMessage="تم نسخ المتبقي" /></span></>)}
        </div>
        <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
          {!editingInvoice && (
            <div style={{ display: "flex", gap: 5 }}>
              <button disabled={!canQuickPay || isPending} onClick={onQuickPay}
                ...
                style={{ flex: 1, height: 32, background: canQuickPay && !isPending ? "linear-gradient(135deg, oklch(0.62 0.18 50), oklch(0.56 0.20 40))" : C.muted, color: canQuickPay && !isPending ? "#fff" : C.mutedFg, border: "none", borderRadius: 6, fontFamily: "inherit", fontSize: 12, fontWeight: 800, cursor: canQuickPay && !isPending ? "pointer" : "not-allowed", display: "flex", alignItems: "center", justifyContent: "center", gap: 4, touchAction: "manipulation" }}>
                <Zap aria-hidden size={13} />دفع سريع
              </button>
              <button disabled={!canReserve || isPending} onClick={onReserve}
                ...
                style={{ flex: 1, height: 32, background: canReserve && !isPending ? "linear-gradient(135deg, oklch(0.58 0.16 230), oklch(0.50 0.18 240))" : C.muted, color: canReserve && !isPending ? "#fff" : C.mutedFg, border: "none", borderRadius: 6, fontFamily: "inherit", fontSize: 12, fontWeight: 800, cursor: canReserve && !isPending ? "pointer" : "not-allowed", display: "flex", alignItems: "center", justifyContent: "center", gap: 4, touchAction: "manipulation" }}>
                <Clock aria-hidden size={13} />حجز الطلب
              </button>
            </div>
          )}
          <button disabled={!canPay || isPending} onClick={onPay}
            ...
            style={{ width: "100%", height: 38, background: canPay && !isPending ? (editingInvoice ? "var(--sem-pos, #10b981)" : C.success) : C.muted, color: canPay && !isPending ? "#fff" : C.mutedFg, border: "none", borderRadius: 7, fontFamily: "inherit", fontSize: 13.5, fontWeight: 900, cursor: canPay && !isPending ? "pointer" : "not-allowed", display: "flex", alignItems: "center", justifyContent: "center", gap: 5, touchAction: "manipulation" }}>
            {isPending
              ? "جارٍ…"
              : !cartLen
                ? "الفاتورة فارغة"
                : editingInvoice
                  ? <><Check aria-hidden size={16} strokeWidth={3} /> اعتماد التعديل البديل</>
                  : isOwing && customerId != null
                    ? <><Check aria-hidden size={16} strokeWidth={3} /> إتمام البيع (آجل) <kbd style={{ background: "rgba(255,255,255,.22)", color: "#fff", borderRadius: 4, padding: "1px 5px", fontFamily: "monospace", fontSize: 9.5, fontWeight: 700 }}>F4</kbd></>
                    : <><Check aria-hidden size={16} strokeWidth={3} /> إتمام الدفع <kbd style={{ background: "rgba(255,255,255,.22)", color: "#fff", borderRadius: 4, padding: "1px 5px", fontFamily: "monospace", fontSize: 9.5, fontWeight: 700 }}>F4</kbd></>}
          </button>
        </div>
        <div style={{ textAlign: "center", marginTop: 3, fontSize: 9.5, color: C.mutedFg, opacity: 0.85 }}>
