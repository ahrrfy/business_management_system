...
export type InTransitRow = RouterOutputs["delivery"]["inTransit"]["rows"][number];
...
export function StaffConfirmDialog({
  ...
  const remaining = Math.max(
    0,
    Number(row.codAmount) -
      Number(row.collectedAmount ?? 0) -
      Number(row.counterSettledAmount ?? 0),
  );
  ...
  const reasonValid =
    !reasonRequired ||
    (shortfallReason !== "" && SHORTFALL_REASONS.includes(shortfallReason as ShortfallReason));
  const canConfirm =
    !pending &&
    noteValid &&
    ...
    >
      ...
      >
        ...
        <p className="mb-3 text-xs leading-relaxed text-muted-foreground">
          سُلِّم الطردُ للزبون. المبلغُ يصير عهدةً على {row.partyName ?? "المندوب"} حتى تُوَرَّده
          لاحقاً في «تسوية المناديب». يُسجَّل التأكيدُ باسمك في سجلّ التدقيق.
        </p>
        <div className="mb-3 grid grid-cols-2 gap-2 rounded-lg border bg-muted/30 p-2 text-xs">
          ...
          <span className="text-end font-black tabular-nums" dir="ltr">
            {fmt(String(remaining))} د.ع
          </span>
        </div>
        ...
        <div className="mb-3 grid grid-cols-2 gap-1.5 rounded-lg border bg-muted/20 p-1">
          <button
            ...
            className={cn(
              ...
              mode === "exact"
                ? "bg-[var(--sem-pos)] text-background shadow-sm"
          ...
          <button
            ...
            className={cn(
              "rounded-md px-3 py-2 text-sm font-bold transition",
              mode === "different"
                ? "bg-[var(--sem-warn)] text-background shadow-sm"
                : "text-muted-foreground hover:bg-accent",
            )}
          >
            مبلغ مختلف
        ...
        {mode === "different" && (
          <>
            <Label htmlFor="staff-amount" className="text-xs">
              المبلغ الذي قبضه المندوب فعلاً
            </Label>
            <div className="mb-3">
              <MoneyInput
                id="staff-amount"
            ...
            {isShort && (
              <>
                ...
                <Label className="text-xs">
                  سبب العجز <span className="text-[var(--sem-neg)]">*</span>
                </Label>
                <div className="mb-3 grid grid-cols-1 gap-1.5">
                  {SHORTFALL_REASONS.map((r) => (
                    ...
                    >
                      <span
                        className="mt-0.5 inline-block size-3 shrink-0 rounded-full border-2"
                        style={{
                          borderColor:
                            shortfallReason === r
                              ? "var(--sem-warn)"
                          ...
                          backgroundColor:
                            shortfallReason === r ? "var(--sem-warn)" : "transparent",
                        }}
                      />
                      <div className="flex-1">
                        <div className="font-bold">{SHORTFALL_REASON_LABEL_AR[r]}</div>
                        <div className="text-muted-foreground">
                          {SHORTFALL_REASON_DESCRIPTION_AR[r]}
                        </div>
                      </div>
                    </button>
                  ))}
        ...
        <Input
          ...
          placeholder="مثلاً: اتصال ٦:٤٥م من المندوب…"
          className="mb-4"
        />
