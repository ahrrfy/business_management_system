...
export function useBarcodeInput(
  ...
  const handleKeyDown = useCallback((event: KeyboardEvent<HTMLInputElement>, setValue: SetInputValue) => {
    ...
    // مسافةٌ يكتبها الإنسان = فاصلُ كلمات في بحثٍ (اسم منتج «عمار السلامي»)، لا جزءٌ من ومضة قارئ.
    // خارجَ ومضةٍ نشطة نمرّرها كما هي ونكسر أيّ مرشّحٍ ناشئ — وإلّا حجبها الالتقاطُ حين تأتي سريعةً بعد
    // حرف (بلاغ المالك ١٥/٩: تعذّر كتابة مسافة البحث). داخلَ ومضةٍ مؤكَّدة تبقى ملتقطةً (باركود المصنع
    // ذو الفراغ لا يزال يُمسَح كما هو).
    if (event.key === " " && !detector.isActive) {
      reset();
      return;
    }
