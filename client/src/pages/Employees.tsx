...
export default function Employees() {
  ...
  /**
   * أعمدة الموظفين.
   * ⚠️ «الموظف» هنا **موضوع** السجلّ لا فاعلَه — فلا يُسمّى «نفّذها»
   *    (راجع حارس check:row-attribution).
   * نقرُ الصفّ يفتح الملفّ؛ وخليّةُ الهاتف توقف الانتشار كي يعمل زرّ النسخ.
   */
  const employeeColumns = useMemo<ColumnDef<Row, unknown>[]>(() => [
    {
            ...
            <div>
              <div className="flex items-center gap-1.5 font-medium">
                ...
                {e.employmentStatus === "active" && !e.deviceLinked && !e.attendanceExempt && (
                  <span
                    className="inline-flex items-center gap-1 rounded-full border border-[var(--sem-warn)]/40 bg-[var(--sem-warn-bg)] px-1.5 py-0.5 text-[10px] font-normal text-[var(--sem-warn)]"
    ...
    { id: "department", header: "القسم", accessorFn: (e) => e.department ?? "—", meta: { kind: "text" } },
    { id: "branchName", header: "الفرع", accessorFn: (e) => e.branchName ?? "—", meta: { kind: "text" } },
    { id: "payType", header: "نوع الأجر", accessorFn: (e) => payTypeLabel(e.payType), meta: { kind: "text" } },
    {
      id: "phone", header: "الهاتف",
      accessorFn: (e) => e.phone ?? "",
      cell: ({ row }) => (
        <span onClick={(ev) => ev.stopPropagation()}>
          <CopyInline value={row.original.phone} />
        </span>
      ),
      meta: { kind: "phone" },
    },
    {
      id: "employmentStatus", header: "الحالة",
      accessorFn: (e) => e.employmentStatus,
      ...
      <Card>
        ...
        <CardContent className="p-0">
          <DataTable
            ...
            /* ترقيمٌ خادميّ ⇒ يُعطَّل الفرز فلا يرتّب صفحةً واحدة ويبدو شاملاً. */
            serverPagination={{ page: displayPage, onPageChange: setPage, pageSize: limit, total }}
