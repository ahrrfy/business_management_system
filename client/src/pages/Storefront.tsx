...
function CustomizationFieldControl({
  ...
  if (field.fieldType === "SELECT") {
    return (
      <AppSelect id={controlId} value={String(value)} onValueChange={(next) => onChange(next)} required={field.isRequired} aria-invalid={invalid || undefined} aria-describedby={describedBy} className={common}>
      ...
      <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-1">
          <label className="relative shrink-0">
            ...
            <AppSelect value={priceFilter} onValueChange={(next) => setPriceFilter(next as PriceFilter)} className="appearance-none -slate-200 bg-white py-1.5 pr-2.5 pl-6 text-[10px] font-bold text-slate-600 outline-none focus:-emerald-400 dark:-slate-700 dark:bg-slate-900 dark:text-slate-300">
      ...
      <main id="store-main" tabIndex={-1} className="mx-auto w-full max-w-[1500px] overflow-x-clip px-4 py-6 pb-28 outline-none lg:px-8">
        ...
        <section id="store-results" className="mt-12 scroll-mt-36 rounded-[28px] bg-white/80 px-4 py-5 shadow-sm ring-1 ring-[#e7e0d8] sm:px-6 sm:py-7">
          ...
          <div className="mb-6 flex flex-col gap-3 border border-[#d9d3ca] bg-white p-3 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex items-center gap-2 overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"><button type="button" onClick={() => { setAvailability((value) => value === "IN_STOCK" ? "ALL" : "IN_STOCK"); scrollToResults(); }} aria-pressed={availability === "IN_STOCK"} className={`shrink-0 border px-3 py-2 text-xs font-black ${availability === "IN_STOCK" ? "border-[#1e4a63] bg-[#1e4a63] text-white" : "border-[#d7d2ca] text-[#59636a]"}`}>{availability === "IN_STOCK" ? "متوفر الآن" : "كل المنتجات"}</button><label className="relative shrink-0"><span className="sr-only">نطاق السعر</span><AppSelect value={priceFilter} onValueChange={(next) => { setPriceFilter(next as PriceFilter); scrollToResults(); }} className="appearance-none -[#d7d2ca] bg-white py-2 pr-3 pl-8 text-xs font-bold text-[#59636a] outline-none focus:-[#1e4a63]"><option value="ALL">كل الأسعار</option><option value="UNDER_5000">أقل من 5,000 د.ع</option><option value="FROM_5000_TO_15000">5,000 – 15,000 د.ع</option><option value="OVER_15000">أكثر من 15,000 د.ع</option></AppSelect><ChevronDown aria-hidden className="pointer-events-none absolute left-2 top-1/2 size-3.5 -translate-y-1/2 text-[#8b9395]" /></label>{brands.length > 0 && <label className="relative shrink-0"><span className="sr-only">الماركة</span><AppSelect value={brand} onValueChange={(next) => { setBrand(next); scrollToResults(); }} className="appearance-none -[#d7d2ca] bg-white py-2 pr-3 pl-8 text-xs font-bold text-[#59636a] outline-none focus:-[#1e4a63]"><option value="">كل الماركات</option>{brands.map((name) => <option key={name} value={name}>{name}</option>)}</AppSelect><ChevronDown aria-hidden className="pointer-events-none absolute left-2 top-1/2 size-3.5 -translate-y-1/2 text-[#8b9395]" /></label>}</div><div className="flex items-center justify-between gap-3"><label className="relative shrink-0"><span className="sr-only">ترتيب النتائج</span><AppSelect value={sort} onValueChange={(next) => { setSort(next as CatalogSort); scrollToResults(); }} className="appearance-none -[#d7d2ca] bg-white py-2 pr-3 pl-8 text-xs font-bold text-[#59636a] outline-none focus:-[#1e4a63]"><option value="RECOMMENDED">الترتيب المقترح</option><option value="BEST_SELLERS">الأكثر مبيعاً</option><option value="PRICE_ASC">السعر: الأقل أولاً</option><option value="PRICE_DESC">السعر: الأعلى أولاً</option></AppSelect><ChevronDown aria-hidden className="pointer-events-none absolute left-2 top-1/2 size-3.5 -translate-y-1/2 text-[#8b9395]" /></label>{(hasRefinements || categoryId != null || search) && <button type="button" onClick={clearCatalogFilters} className="text-xs font-black text-[#a4513f] hover:underline">{FILTER_LABELS.reset}</button>}</div>
          </div>
        ...
        <PanelShell title="إتمام الطلب" onClose={() => {
          ...
          <form noValidate onSubmit={(event) => { event.preventDefault(); submitOrder(); }} className="flex flex-col gap-3">
            ...
            <Field icon={<Store aria-hidden className="size-4" />} label="المحافظة" htmlFor="storefront-checkout-governorate" required error={checkoutErrors.governorate}>
              <AppSelect id="storefront-checkout-governorate" value={String(form.governorate)} onValueChange={(next) => updateCheckoutField("governorate", next)} required aria-invalid={Boolean(checkoutErrors.governorate)} aria-describedby={checkoutErrors.governorate ? "storefront-checkout-governorate-error" : undefined} className="text-sm outline-none">
