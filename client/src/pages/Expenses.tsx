...
const AUDIT_WARNING_LABEL: Record<string, string> = {
  ...
  ACCRUAL_PAYMENT_METHOD_MISMATCH: "طريقة المصروف المستحق لا تطابق عقد الاستحقاق",
  ACCRUAL_HAS_DIRECT_RECEIPT: "الاعتراف بالاستحقاق مرتبط خطأً بإيصال نقدي مباشر",
  ...
  UNPAID_ACCRUAL_HAS_SETTLEMENT_EFFECT: "استحقاق غير مدفوع يحمل أثر تسوية متناقضاً",
};
...
function ExpenseTracePanel({ expenseId }: { expenseId: number }) {
  ...
  const settlementReceiptId =
    expense.settlementReceiptId == null ? null : Number(expense.settlementReceiptId);
    ...
    <FinancialTraceDetails
      ...
      document={{
        ...
        extra: [
          ...
          { label: "مرجع دليل المصدر", value: expense.accrualEvidenceReference },
        ],
      }}
      parties={{
        recordedBy: {
      ...
      moneyPath={{
        ...
        extra: [
          ...
          { label: "قيد الاعتراف", value: expense.recognitionAccountingEntryId ? `JE#${expense.recognitionAccountingEntryId}` : null },
          { label: "قيد التسوية", value: expense.settlementAccountingEntryId ? `JE#${expense.settlementAccountingEntryId}` : null },
        ],
      }}
      linkChain={[
        {
        ...
        ...(expense.source === "ACCRUAL"
          ? obligationEvents.map((event) => ({
              ...
              kind: (event.accountingEntryId ? "LEDGER" : event.receiptId ? "RECEIPT" : "DOCUMENT") as "LEDGER" | "RECEIPT" | "DOCUMENT",
              label:
      ...
      auditTimeline={[
        ...
        ...trace.data.correctionRequests.map((event) => ({
          ...
          tone: event.status === "APPROVED" ? "ok" as const : event.status === "REJECTED" ? "warning" as const : "info" as const,
        })),
      ]}
    />
  );
...
async function printExpenseReceipt(r: ExpenseRow) {
  if (r.source === "ACCRUAL") {
    notify.err("المصروف المستحق ليس إيصال صرف؛ اطبع سند التسوية الفعلي من شاشة السندات بعد اعتماده");
    return;
  }
...
export default function Expenses() {
  ...
  const [correctionTarget, setCorrectionTarget] = useState<ExpenseRow | null>(null);
  ...
  const exportColumns: ExportColumn<ExpenseRow>[] = [
    ...
    { key: "settlementStatus", header: "حالة تسوية الاستحقاق", map: (r) => r.settlementStatus ?? "" },
    { key: "accrualObligationId", header: "رقم التزام الاستحقاق", map: (r) => r.accrualObligationId ?? "" },
    { key: "accrualEvidenceReference", header: "مرجع دليل الاعتراف", map: (r) => r.accrualEvidenceReference ?? "" },
    { key: "recognitionAccountingEntryId", header: "قيد الاعتراف", map: (r) => r.recognitionAccountingEntryId ?? "" },
    { key: "settlementReceiptId", header: "سند التسوية", map: (r) => r.settlementReceiptId ?? "" },
    { key: "settlementAccountingEntryId", header: "قيد التسوية", map: (r) => r.settlementAccountingEntryId ?? "" },
  ...
  const requestCorrection = trpc.expenses.requestAccrualCorrection.useMutation({
    onSuccess: async () => {
      ...
      await Promise.all([utils.expenses.list.invalidate(), correctionHistory.refetch()]);
    },
    onError: (error) => notify.err(error),
  });
    ...
    return [
      {
        ...
        hidden: !isExpenseFinanciallyPrintable(r.status) || r.source === "ACCRUAL",
      ...
      },
      {
        ...
        hidden:
          ...
          !["ACCRUED_UNPAID", "PAYMENT_PENDING", "PAID", "CORRECTION_PENDING", "REFUND_PENDING"].includes(r.settlementStatus ?? "") ||
          !canCancel,
  ...
  const activeCorrection = correctionHistory.data?.find((item) => item.status === "PENDING") ?? null;
  const correctionRequiresRefund = correctionTarget?.settlementStatus === "PAID";
  ...
  return (
    <div className="space-y-3">
        ...
        <Card className="gap-0 overflow-hidden py-0">
          <CardContent className="flex items-start justify-between gap-3 p-3">
            <div>
              ...
              <p className="mt-0.5 text-[11px] text-muted-foreground">
                مستحق غير مدفوع: <span dir="ltr">{metric(totals.accruedUnpaid)}</span>
              </p>
            </div>
        ...
        <Card className="gap-0 overflow-hidden py-0">
          <CardContent className="flex items-start justify-between gap-3 p-3">
            <div>
              ...
              <p className="mt-0.5 text-[11px] text-muted-foreground">
                استحقاق مسوّى: <span dir="ltr">{metric(totals.accruedPaid)}</span>
              </p>
            </div>
      ...
      <Dialog
        ...
        onOpenChange={(open) => {
          if (!open && !requestCorrection.isPending && !approveCorrection.isPending && !rejectCorrection.isPending) {
      ...
      >
        <DialogContent dir="rtl" className="max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              التصحيح يحفظ المصروف الأصلي ويضيف طلباً وحدثاً وعكساً مستقلاً. المصروف المدفوع يتطلب استرداداً فعلياً موثقاً واعتماد مالك آخر.
            </DialogDescription>
          </DialogHeader>
          {correctionTarget && (
            <div className="space-y-3">
              <div className="rounded-md border p-3 text-sm">
                <div className="font-medium">EXP#{Number(correctionTarget.id)} · {fmt(correctionTarget.amount)} د.ع</div>
                <div className="mt-1 text-muted-foreground">{correctionTarget.accrualBeneficiaryName ?? correctionTarget.payee ?? "مستفيد غير موثق"} · {FUNDING_META[fundingKindOf(correctionTarget)].short}</div>
                <div className="mt-1 text-xs" dir="ltr">{correctionTarget.accrualEvidenceReference ?? "—"}</div>
              </div>

              {correctionHistory.isLoading ? (
                <div className="flex items-center gap-2 text-sm text-muted-foreground"><Loader2 aria-hidden className="size-4 animate-spin" /> جارٍ تحميل سجل التصحيح…</div>
              ) : activeCorrection ? (
                <div className="space-y-3 rounded-md border badge-status-pending p-3 text-sm">
                  <div className="font-medium">طلب تصحيح #{activeCorrection.id} بانتظار الاعتماد</div>
                  <p>{activeCorrection.reason}</p>
                  <p className="text-xs" dir="ltr">{activeCorrection.externalEvidenceReference}</p>
                  {activeCorrection.previousObligationStatus === "PAID" ? (
                    <p>أُنشئ طلب قبض استرداد معلّق بلا أثر نقدي. تتم المراجعة من شاشة سندات القبض لمطابقة دليل المزود قبل أي قيد.</p>
                  ) : me.data?.isOwner === true && Number(activeCorrection.requestedBy) !== Number(me.data.id) ? (
                    <div className="space-y-2 border-t pt-3">
                      <Label htmlFor="expense-correction-review-reason">سبب الرفض عند الرفض</Label>
                      <Input id="expense-correction-review-reason" value={correctionReviewReason} onChange={(event) => setCorrectionReviewReason(event.target.value)} placeholder="يُترك فارغاً عند الاعتماد" />
                      <div className="flex gap-2">
                        <Button disabled={approveCorrection.isPending || rejectCorrection.isPending} onClick={async () => {
                          if (!(await confirm({ variant: "warning", title: "اعتماد تصحيح المصدر", description: "سيُغلق المصدر التشغيلي ويُعكس قيد الاعتراف بقيد append-only مستقل.", confirmText: "اعتماد التصحيح" }))) return;
                          approveCorrection.mutate({ correctionRequestId: Number(activeCorrection.id) });
                        }}>اعتماد التصحيح</Button>
                        <Button variant="destructive" disabled={correctionReviewReason.trim().length < 3 || approveCorrection.isPending || rejectCorrection.isPending} onClick={() => rejectCorrection.mutate({ correctionRequestId: Number(activeCorrection.id), reason: correctionReviewReason.trim() })}>رفض التصحيح</Button>
                      </div>
                    </div>
                  ) : (
                    <p className="text-muted-foreground">ينتظر مالكاً آخر؛ لا يستطيع منشئ الطلب اعتماده أو رفضه.</p>
                  )}
                </div>
              ) : (
                <>
                  <div className="space-y-1">
                    <Label htmlFor="expense-correction-reason">سبب التصحيح *</Label>
                    <Textarea id="expense-correction-reason" rows={3} value={correctionReason} onChange={(event) => setCorrectionReason(event.target.value)} />
                  </div>
                  <div className="space-y-1">
                    <Label htmlFor="expense-correction-evidence">مرجع الدليل الخارجي *</Label>
                    <Input id="expense-correction-evidence" dir="ltr" value={correctionEvidence} onChange={(event) => setCorrectionEvidence(event.target.value)} />
                  </div>
                  <ImageUploader value={correctionAttachment} onChange={setCorrectionAttachment} maxItems={1} singlePrimary={false} hint="مرفق فاتورة التصحيح/الإشعار الدائن إلزامي." />
                  {correctionRequiresRefund && (
                    <div className="space-y-3 rounded-md border p-3">
                      <p className="font-medium">دليل الاسترداد الفعلي</p>
                      <div className="grid grid-cols-2 gap-3">
                        <div className="space-y-1"><Label>الطريقة</Label><select className={selectCls} value={correctionRefundMethod} onChange={(event) => setCorrectionRefundMethod(event.target.value as typeof correctionRefundMethod)}><option value="CASH">نقد</option><option value="CARD">بطاقة</option><option value="CHECK">صك</option><option value="TRANSFER">تحويل</option><option value="WALLET">محفظة</option></select></div>
                        {correctionRefundMethod === "CASH" ? (
                          <div className="space-y-1"><Label>وجهة النقد</Label><select className={selectCls} value={correctionRefundBucket} onChange={(event) => setCorrectionRefundBucket(event.target.value as typeof correctionRefundBucket)}><option value="TREASURY">الخزينة الإدارية</option><option value="DRAWER">درج الوردية</option></select></div>
                        ) : (
                          <div className="space-y-1"><Label>مرجع مزود الاسترداد</Label><Input dir="ltr" value={correctionRefundReference} onChange={(event) => setCorrectionRefundReference(event.target.value)} /></div>
                        )}
                        {correctionRefundMethod === "CARD" && <div className="space-y-1"><Label>آخر أربعة أرقام</Label><Input dir="ltr" maxLength={4} value={correctionRefundCardTail} onChange={(event) => setCorrectionRefundCardTail(event.target.value.replace(/\D/g, "").slice(0, 4))} /></div>}
                      </div>
                    </div>
                  )}
                </>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setCorrectionTarget(null)}>إغلاق</Button>
            {!activeCorrection && correctionTarget && (
              <Button
                variant="destructive"
                disabled={
                  correctionHistory.isLoading ||
                  ...
                  (correctionRequiresRefund && correctionRefundMethod !== "CASH" && !correctionRefundReference.trim()) ||
                  (correctionRequiresRefund && correctionRefundMethod === "CARD" && !/^\d{4}$/.test(correctionRefundCardTail))
                }
                onClick={() => requestCorrection.mutate({
                  obligationId: Number(correctionTarget.accrualObligationId),
                  reason: correctionReason.trim(),
                  externalEvidenceReference: correctionEvidence.trim(),
                  attachmentUrl: correctionAttachment[0]!.dataUrl,
                  refundPaymentMethod: correctionRequiresRefund ? correctionRefundMethod : null,
                  refundCashBucket: correctionRequiresRefund && correctionRefundMethod === "CASH" ? correctionRefundBucket : null,
                  refundReferenceNumber: correctionRequiresRefund && correctionRefundMethod !== "CASH" ? correctionRefundReference.trim() : null,
                  refundCardLastFour: correctionRequiresRefund && correctionRefundMethod === "CARD" ? correctionRefundCardTail : null,
                  clientRequestId: correctionClientRequestId,
                })}
              >
                {requestCorrection.isPending ? <Loader2 aria-hidden className="size-4 animate-spin" /> : null}
                تسجيل طلب التصحيح
              </Button>
            )}
          </DialogFooter>
