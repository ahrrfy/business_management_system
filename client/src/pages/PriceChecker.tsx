/**
 * شاشة «قارئ الأسعار» (الكشك) — تُعرض للزبون بملء الشاشة (بلا قائمة جانبية).
 *
 * وضعان: (١) الجذب — بنر يدور على المنتجات المتوفّرة في الفرع (سعر المفرد + صورة)؛
 * (٢) المسح — الماسح (لوحة مفاتيح) يُظهر بطاقة سعر ضخمة ثم تعود تلقائياً. وحالة «غير موجود».
 * البيانات آمنة للزبون (kioskRouter): بلا تكلفة ولا كمية مخزون.
 * لوحة الإعدادات/المحاكاة أسفل اليسار للموظّف فقط (اختيار الفرع/الثيم/تجربة المسح).
 */
...
export default function PriceChecker() {
  ...
  const branchId = settings.branchId ?? me.data?.branchId ?? branches[0]?.id ?? null;
  const branchName = branches.find((b) => b.id === branchId)?.name ?? "—";

  const bannerQ = trpc.kiosk.banner.useQuery(
    { branchId: branchId ?? 0, limit: 40 },
    { enabled: branchId != null, refetchInterval: 5 * 60 * 1000, refetchOnWindowFocus: false }
  );
  ...
  const handleScan = useCallback(async (code: string) => {
    ...
    if (!clean || branchId == null) return;
    try {
      const p = (await utils.kiosk.lookup.fetch({ branchId, barcode: clean })) as KProduct | null;
  ...
  return (
    <div className={"kioskpc-root" + (settings.theme === "dark" ? " kpc-dark" : "")}>
      ...
      {/* أدوات الموظّف (خارج اللوحة المُحجَّمة) — إعدادات + محاكاة المسح */}
      <div className="kpc-tools">
        <button className="kpc-fab" onClick={() => setPanelOpen((v) => !v)} title="إعدادات الكشك">
          <IconScan s={20} /> إعدادات الكشك
        </button>
      </div>
      ...
      {panelOpen && (
        <div className="kpc-panel">
          <div className="kpc-panel-head">
            <strong>إعدادات الكشك</strong>
            <button onClick={() => setPanelOpen(false)} aria-label="إغلاق">✕</button>
          </div>
          <div className="kpc-panel-body">
            <div className="kpc-field">
              <label>الفرع</label>
              <select value={branchId ?? ""} onChange={(e) => setTweak("branchId", e.target.value ? Number(e.target.value) : null)}>
