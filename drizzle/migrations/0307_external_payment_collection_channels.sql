...
-- MySQL may use the existing unique invoice index to satisfy fk_extpay_invoice.
-- Create its non-unique replacement first so dropping uniqueness never removes
-- the only supporting index for that foreign key.
