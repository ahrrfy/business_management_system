...
import { formatIqd, storefrontDisplayPrice } from "@/lib/storefront-api";
...
export function SideCart({ visible, onClose }: { visible: boolean; onClose: () => void }) {
  ...
  const subtotal = lines.reduce((sum, line) => sum + (Number(storefrontDisplayPrice(line.product) ?? 0) || 0) * line.quantity, 0);
  ...
  return <Modal animationType="none" onRequestClose={onClose} statusBarTranslucent transparent visible>
    <View style={styles.layer}>
      ...
      <Animated.View style={[styles.drawer, { paddingBottom: Math.max(insets.bottom, 12), paddingTop: Math.max(insets.top, 14), transform: [{ translateX }] }]}>
        ...
        {lines.length ? <ScrollView contentContainerStyle={styles.lines} showsVerticalScrollIndicator={false}>{lines.map((line) => <View key={line.product.id} style={styles.line}><View style={styles.lineCopy}><Text numberOfLines={1} style={styles.lineTitle}>{line.product.title}</Text><Text style={styles.linePrice}>{formatIqd(storefrontDisplayPrice(line.product))}</Text></View><View style={styles.quantity}><TouchableOpacity activeOpacity={0.8} onPress={() => decrement(line.product.id)} style={styles.quantityButton}><MaterialIcons color="#0E806A" name="remove" size={16} /></TouchableOpacity><Text style={styles.quantityText}>{line.quantity}</Text><TouchableOpacity activeOpacity={0.8} onPress={() => increment(line.product.id)} style={styles.quantityButton}><MaterialIcons color="#0E806A" name="add" size={16} /></TouchableOpacity></View><TouchableOpacity accessibilityLabel={`حذف ${line.product.title}`} activeOpacity={0.8} onPress={() => remove(line.product.id)} style={styles.remove}><MaterialIcons color="#A25A50" name="close" size={15} /></TouchableOpacity></View>)}</ScrollView> : <View style={styles.empty}><MaterialIcons color="#9AB7AC" name="shopping-basket" size={40} /><Text style={styles.emptyTitle}>السلة فارغة حالياً</Text><Text style={styles.emptyHint}>أضف ما يعجبك وستظهر هنا فوراً.</Text></View>}
