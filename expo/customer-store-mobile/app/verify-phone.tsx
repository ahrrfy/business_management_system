  ...
  return (
    <ScreenContainer className="flex-1" containerClassName="bg-background">
      <View style={styles.content}>
        <TouchableOpacity onPress={() => router.back()} style={styles.close} accessibilityLabel="رجوع"><MaterialIcons name="close" size={23} color="#52655D" /></TouchableOpacity>
        ...
        {step === "PHONE" ? <>
          ...
          <TouchableOpacity disabled={busy} onPress={sendCode} style={[styles.primary, busy && styles.disabled]}>{busy ? <ActivityIndicator color="#FFFFFF" /> : <Text style={styles.primaryText}>إرسال رمز التحقق</Text>}</TouchableOpacity>
        </> : <>
          ...
          <TouchableOpacity disabled={busy} onPress={verifyCode} style={[styles.primary, busy && styles.disabled]}>{busy ? <ActivityIndicator color="#FFFFFF" /> : <Text style={styles.primaryText}>تأكيد الهاتف</Text>}</TouchableOpacity>
          <TouchableOpacity disabled={busy} onPress={() => { setStep("PHONE"); setCode(""); }} style={styles.secondary}><Text style={styles.secondaryText}>تعديل الرقم أو إعادة الإرسال</Text></TouchableOpacity>
        </>}
