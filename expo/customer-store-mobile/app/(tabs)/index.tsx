...
const DISCOVERY_GUIDE = [
  { audience: "طالب", direction: "ابحث باسم الكتاب أو المادة.", icon: "school", route: "/search" },
  { audience: "فرد", direction: "تصفّح الأقسام حسب حاجتك.", icon: "person-outline", route: "/categories" },
  { audience: "مكتب", direction: "راجع الأقسام لمستلزمات العمل.", icon: "business-center", route: "/categories" },
  { audience: "شركة", direction: "أرسل طلب عرض لاحتياجاتك.", icon: "corporate-fare", route: "/request-quote" },
] as const;
...
export default function HomeScreen() {
  ...
  return <ScreenContainer className="flex-1" containerClassName="bg-background">
    <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.guideSection}><Text style={styles.guideTitle}>دليل سريع لاكتشاف الكتالوج</Text><View style={styles.guideGrid}>{DISCOVERY_GUIDE.map((item) => <TouchableOpacity accessibilityLabel={`مسار ${item.audience}: ${item.direction}`} accessibilityRole="button" activeOpacity={0.85} key={item.audience} onPress={() => router.push(item.route as never)} style={styles.guideCard}><MaterialIcons color="#0E806A" name={item.icon} size={21} /><View style={styles.guideCopy}><Text style={styles.guideAudience}>{item.audience}</Text><Text style={styles.guideDirection}>{item.direction}</Text></View><MaterialIcons color="#8AA49B" name="arrow-back" size={16} /></TouchableOpacity>)}</View></View>
...
const styles = StyleSheet.create({
  ...
  guideSection: { marginTop: 22 }, guideTitle: { color: "#183D36", fontFamily: "Cairo_800ExtraBold", fontSize: 16, textAlign: "right" }, guideGrid: { gap: 9, marginTop: 10 }, guideCard: { alignItems: "center", backgroundColor: "#FFFFFF", borderColor: "#EEE3D7", borderRadius: 14, borderWidth: 1, flexDirection: "row-reverse", gap: 10, minHeight: 67, paddingHorizontal: 12, paddingVertical: 10 }, guideCopy: { flex: 1 }, guideAudience: { color: "#183D36", fontFamily: "Cairo_700Bold", fontSize: 12, textAlign: "right" }, guideDirection: { color: "#6D817A", fontFamily: "Cairo_400Regular", fontSize: 10, marginTop: 2, textAlign: "right" },
