...
import { formatLatinNumber, getStorefrontCustomerBenefits, requestStorefrontFirstOrderCoupon, type StorefrontCustomerBenefits } from "@/lib/storefront-api";
...
export default function LoyaltyScreen() {
  ...
  const requestFirstOrderCoupon = async () => {
    ...
    try {
      ...
      Alert.alert("تعذر طلب الكوبون", error instanceof Error ? error.message : "أعد المحاولة لاحقاً.");
    } finally {
  ...
  return <ScreenContainer className="flex-1" containerClassName="bg-background"><ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}><TouchableOpacity onPress={() => router.back()} style={styles.back}><MaterialIcons name="arrow-forward" size={22} color="#40594F" /></TouchableOpacity><Text style={styles.title}>الولاء والقسائم</Text><Text style={styles.sub}>مرحباً {benefits?.customer.name ?? session.customer.name}</Text>
    ...
    <View style={benefitPolicyStyles.card}><View style={benefitPolicyStyles.heading}><MaterialIcons name="local-offer" size={20} color="#614914" /><Text style={benefitPolicyStyles.title}>سياسة التوفير في الطلب</Text></View><Text style={benefitPolicyStyles.summary}>نطبّق منفعة سعرية واحدة فقط على المنتجات في كل طلب: سعر الجملة أو العرض أو الكوبون.</Text><Text style={benefitPolicyStyles.detail}>عند اجتماعها، يُعتمد الأعلى توفيراً تلقائياً. عرض التوصيل مستقل عن هذه المنفعة.</Text></View>
    <View style={firstOrderStyles.card}><Text style={firstOrderStyles.title}>كوبون الطلب الأول</Text><Text style={firstOrderStyles.hint}>اطلبه بنفسك قبل إنشاء أول طلب. نتحقق من أهليتك ونصدره مرة واحدة فقط عند تفعيل الإدارة للبرنامج.</Text><TouchableOpacity accessibilityRole="button" disabled={requestingFirstOrderCoupon} onPress={() => void requestFirstOrderCoupon()} style={[firstOrderStyles.button, requestingFirstOrderCoupon && firstOrderStyles.buttonDisabled]}>{requestingFirstOrderCoupon ? <ActivityIndicator color="#FFFFFF" size="small" /> : <Text style={firstOrderStyles.buttonText}>طلب كوبون الطلب الأول</Text>}</TouchableOpacity></View>
...
const benefitPolicyStyles = StyleSheet.create({
  card: { backgroundColor: "#FFF7E7", borderColor: "#F2DCAD", borderRadius: 14, borderWidth: 1, marginTop: 13, padding: 13 },
  heading: { alignItems: "center", flexDirection: "row-reverse", gap: 7 },
  title: { color: "#614914", flex: 1, fontSize: 14, fontWeight: "900", textAlign: "right" },
...
const firstOrderStyles = StyleSheet.create({
  card: { backgroundColor: "#FFF7E7", borderColor: "#F2DCAD", borderRadius: 14, borderWidth: 1, marginTop: 13, padding: 13 },
  title: { color: "#614914", fontSize: 14, fontWeight: "900", textAlign: "right" },
  hint: { color: "#746130", fontSize: 11, lineHeight: 18, marginTop: 5, textAlign: "right" },
  button: { alignItems: "center", backgroundColor: "#0C5A4B", borderRadius: 10, justifyContent: "center", marginTop: 10, minHeight: 40 },
