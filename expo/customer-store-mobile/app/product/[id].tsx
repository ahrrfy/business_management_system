...
export default function ProductDetailScreen() {
  ...
  useEffect(() => {
    if (!product) return;
    const variant =
      product.variants?.find((candidate) => candidate.inStock) ??
      product.variants?.[0];
    const unit =
      variant?.units.find((candidate) => candidate.inStock) ??
      variant?.units[0];
  ...
  const selectedVariant = product?.variants?.find(
    (candidate) => candidate.variantId === selectedVariantId,
  );
  const selectedUnit = selectedVariant?.units.find(
    (candidate) => candidate.productUnitId === selectedUnitId,
  );
  ...
  if (loading)
    return (
      ...
      >
        <ActivityIndicator
          accessibilityLabel="جار تحميل المنتج"
          color="#0E806A"
          size="large"
        />
        <Text style={styles.stateText}>جار تحميل المنتج…</Text>
      </ScreenContainer>
    );
  if (!product)
    return (
      <ScreenContainer
        ...
        containerClassName="bg-background"
      >
        <MaterialIcons color="#A25A50" name="cloud-off" size={36} />
        <Text accessibilityRole="alert" style={styles.stateTitle}>
          {error ?? "المنتج غير متاح"}
        </Text>
        <Text style={styles.stateText}>
          تحقق من الاتصال ثم عد إلى المنتجات وحاول مجدداً.
        </Text>
        <TouchableOpacity
          accessibilityRole="button"
          onPress={() => router.back()}
          style={styles.backSimple}
        >
          <Text style={styles.backSimpleText}>العودة للمتجر</Text>
        </TouchableOpacity>
      </ScreenContainer>
    );
  ...
  const validation = validateProductSelection(product, {
    variantId: selectedVariantId,
    productUnitId: selectedUnitId,
    customizationValues,
  });
  const currentLineId = validation.details
    ? cartLineKey(product.productId ?? product.id, validation.details)
    : null;
    ...
    <ScreenContainer className="flex-1" containerClassName="bg-background">
      ...
      >
        <View style={styles.topbar}>
          ...
          >
            <MaterialIcons color="#0E806A" name="arrow-forward" size={23} />
          </TouchableOpacity>
          ...
          <TouchableOpacity
            ...
            style={styles.iconButton}
          >
            <MaterialIcons color="#0E806A" name="shopping-bag" size={21} />
          </TouchableOpacity>
        </View>
        ...
        <View style={styles.galleryWrap}>
          {gallery.length ? (
            ...
            >
              {gallery.map((uri) => (
                <View
                  ...
                  style={[
                    styles.cover,
                    { width: galleryWidth, backgroundColor: product.accent },
                  ]}
                >
                  <Image
                    accessibilityLabel={`صورة ${product.title}`}
            ...
            >
              <MaterialIcons color="#0E806A" name={product.icon} size={96} />
            </View>
          )}
          <Text style={styles.galleryLabel}>
            {gallery.length
              ? `صورة ${selectedImage + 1} من ${gallery.length}`
              : "لا توجد صورة معتمدة"}
          </Text>
        </View>
        ...
        <View style={styles.card}>
          <View style={styles.metaRow}>
            ...
            {product.hasAlternatives && (
              <Text style={styles.alternativeBadge}>بدائل متاحة</Text>
            )}
          </View>
          <Text accessibilityRole="header" style={styles.title}>
            {product.title}
          </Text>
          <Text style={styles.brand}>{product.brand ?? product.subtitle}</Text>
        ...
        {onlineOrderingIssue ? (
          ...
          >
            <MaterialIcons color="#A25A50" name="info-outline" size={22} />
            <View style={styles.unavailableCopy}>
              <Text style={styles.unavailableTitle}>
                طلب التخصيص متوقف إلكترونياً مؤقتاً
              </Text>
        ...
        {!onlineOrderingIssue && product.customizationTemplate && (
          <View style={styles.card}>
            ...
            {product.customizationTemplate.description && (
              <Text style={styles.helpText}>
                {product.customizationTemplate.description}
              </Text>
            )}
            {activeFields.map((field) => (
              <CustomizationFieldInput
                field={field}
                key={field.fieldKey}
                onChange={(value) => {
                ...
                value={customizationValues[field.fieldKey] ?? ""}
              />
            ))}
          </View>
        )}
        {product.bundleItems && product.bundleItems.length > 0 && (
          <View style={styles.card}>
            <Text style={styles.sectionTitle}>محتويات المجموعة</Text>
            {product.bundleItems.map((item) => (
              <Text key={item.name} style={styles.bundleItem}>
                • {item.name} × {formatLatinNumber(item.quantity)}
              </Text>
            ))}
          </View>
        )}
        ...
        <View style={styles.trust}>
          <MaterialIcons color="#0E806A" name="verified-user" size={20} />
          <Text style={styles.trustText}>
            يعاد فحص السعر والمخزون من نظام المكتبة قبل إرسال الطلب.
          </Text>
        </View>
        ...
        {!onlineOrderingIssue && (
          <View style={styles.purchaseActions}>
            <TouchableOpacity
              ...
              accessibilityRole="button"
              disabled={!selectedUnit?.inStock}
              onPress={addToCart}
              style={[
                styles.addButton,
                !selectedUnit?.inStock && styles.addDisabled,
              ]}
            >
            ...
            <TouchableOpacity
              accessibilityHint="يحفظ اختيارك ثم ينتقل إلى إتمام الطلب"
              accessibilityLabel="اشتر الآن"
              accessibilityRole="button"
              disabled={!selectedUnit?.inStock}
              onPress={buyNow}
              style={[
...
function CustomizationFieldInput({
  field,
  onChange,
  value,
}: {
    ...
    <View style={styles.field}>
      ...
      {field.fieldType === "FILE" ? (
        ...
        <TextInput
          ...
          maxLength={field.maxLength ?? DEFAULT_CUSTOMIZATION_VALUE_MAX_LENGTH}
          ...
          style={[
            styles.input,
            field.fieldType === "TEXTAREA" && styles.textarea,
          ]}
          textAlign="right"
          value={value}
        />
...
const styles = StyleSheet.create({
  ...
  cover: { alignItems: "center", height: 320, justifyContent: "center" },
  ...
  card: {
    backgroundColor: storefrontDesign.semantic.surface,
    borderColor: storefrontDesign.semantic.border,
    borderRadius: 24,
    borderWidth: 1,
    marginTop: 13,
  ...
  statusUnavailable: { backgroundColor: "#FFF0EC" },
  statusText: {
    color: storefrontDesign.semantic.brandStrong,
    fontFamily: "Cairo_700Bold",
    fontSize: 10,
  },
  statusUnavailableText: { color: "#A25A50" },
  alternativeBadge: {
