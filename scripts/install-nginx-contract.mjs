...
function decodeReleaseHash(chunks) {
  if (
    !Array.isArray(chunks) ||
    chunks.length !== 4 ||
    chunks.some((chunk) => !/^[a-f0-9]{16}$/u.test(chunk))
  ) {
    return null;
  }
  return chunks.join("");
...
function assertReleaseHash(content, relative, manifest) {
  if (!manifest) return;
  const expected = decodeReleaseHash(manifest.files[relative]);
...
function verifyReleaseInputs(contract, options) {
  ...
  for (const relative of expectedKeys) {
    if (!decodeReleaseHash(manifest.files[relative])) {
      throw installerError("NGINX_RELEASE_INPUT_HASH_MISMATCH");
    }
  }
